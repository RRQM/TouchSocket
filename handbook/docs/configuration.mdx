...
`ASP.NET Core` 和 `TouchSocket` 提供了多种配置类型读取并转换成特定类型，包括基础类型，对象类型，数组，集合，字典等等，如：
...
```cs showLineNumbers
var companyName = App.Configuration["AppInfo:Name"]; // => TouchSocket
```
...
`TouchSocket v2.8.0` 版本移除了 `.xml` 文件自动扫描配置了，改为手动添加配置。

:::

大多情况下，我们的配置只需要在 `appsettings.json` 中配置即可，但一些特殊情况下，我们希望某些组件或功能拥有独立的配置，这个时候就需要用到自定义配置，`TouchSocket` 目前支持 `.json` 和 `.xml` 两种方式配置，如：

```json showLineNumbers  {2-13} title="TouchSocket.Web.Entry/emailsetting.json"
{
  "outlook": {
    "smtp": {
...
`TouchSocket` 框架会在启动时自动扫描每一个项目层根目录下的 `*.json` 文件加入配置中，所以无需手工配置。
...
同时 `TouchSocket` 提供了非常灵活的方式支持自定义配置文件读取，如：
...
`TouchSocket` 会在应用启动的时候对 `IConfiguration` 进行静态缓存，如果使用了 `App.Configuration` 静态属性且配置数据已发生变更，则调用以下方法刷新即可：
