...
`TouchSocket` 框架编写的所有后端接口都会生成规范化的 `swagger.json` 文件，使用该文件可以在 **[https://editor.swagger.io](https://editor.swagger.io)** 生成任何支持标准 `swagger` 的界面或客户端代码。
...
`TouchSocket` 也提供了 **[Swagger-Editor.rar](https://gitee.com/rrqm_home/TouchSocket/blob/v4/clients/Swagger-Editor.rar)** 离线包，可直接下载解压并双击 `index.html` 即可。
...
**接着下载 `TouchSocket` 提供的 `Vue/React` 工具库 `axios-utils.ts` 并拷贝到和 `api-services` 同级目录下：**

[axios-utils.ts 下载地址](https://gitee.com/rrqm_home/TouchSocket/blob/v4/clients/axios_vue_react/axios-utils.ts)
...
**接着下载 `TouchSocket` 提供的 `Angular` 工具库 `angular-utils.ts` 并拷贝到和 `api-services` 同级目录下：**

[angular-utils.ts 下载地址](https://gitee.com/rrqm_home/TouchSocket/blob/v4/clients/angular/angular-utils.ts)
...
在 `Vue/React` 项目中编辑 `TouchSocket` 框架提供的 `axios-utils.ts` 文件，并将 `serveConfig` 修改为对应的后端地址即可，如：
...
如果是 `Angular` 项目则编辑 `TouchSocket` 框架提供的 `angular-utils.ts` 文件，并将 `serveConfig` 修改为对应的后端地址即可，如：
...
`axios-utils.ts` 和 `angular-utils.ts` 是 `TouchSocket` 框架专门针对 `TouchSocket` 开发的 `WebAPI` 项目编写的客户端代理库，在这个代理库中已经处理了跨域，授权，自动刷新 `token` 以及解密客户端 `JWT token` 问题。
...
`TouchSocket` 官网也提供了 `Swagger-Editor.rar` 离线包下载：[https://gitee.com/rrqm_home/TouchSocket/blob/v4/clients/Swagger-Editor.rar](https://gitee.com/rrqm_home/TouchSocket/blob/v4/clients/Swagger-Editor.rar)
