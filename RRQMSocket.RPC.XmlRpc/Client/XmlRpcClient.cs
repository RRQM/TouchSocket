...
namespace RRQMSocket.RPC.XmlRpc
{
    ...
    /// XmlRpc客户端
    /// </summary>
    public class XmlRpcClient : HttpClientBase, IRpcClient
    {
        /// <summary>
        /// Rpc调用
        /// </summary>
        ...
        public T Invoke<T>(string method, IInvokeOption invokeOption, ref object[] parameters, Type[] types)
        {
            ...
            using (ByteBlock byteBlock = new ByteBlock(this.BufferLength))
            {
                ...
                HttpResponse response = this.Request(request, invokeOption.FeedbackType == FeedbackType.OnlySend, invokeOption.Timeout, invokeOption.Token);
        ...
        public void Invoke(string method, IInvokeOption invokeOption, ref object[] parameters, Type[] types)
        {
            ...
            {
                HttpRequest request = XmlDataTool.CreateRequest(this.RemoteIPHost.Host, this.RemoteIPHost.GetUrlPath(), method, parameters);
                var response = this.Request(request, invokeOption.FeedbackType == FeedbackType.OnlySend, invokeOption.Timeout, invokeOption.Token);
                if (invokeOption.FeedbackType != FeedbackType.WaitInvoke)
                {
                    return;
                }
