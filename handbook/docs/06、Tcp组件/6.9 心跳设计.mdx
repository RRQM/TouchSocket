...
其实TCP中已经为我们实现了一个[内置心跳机制（SetKeepAliveValue）](https://www.yuque.com/rrqm/touchsocket/436f0f670b44744c9f1082d97ced2613#cmsde)。但是该机制受限于操作系统，而且很容易误报。所以很少被大家使用。
...
## 二、设计数据格式
使用心跳之前，必须要明确数据格式，绝对不能混淆业务数据。一般在适配Plc等现成模块时，他们是有固定的数据格式，这时候你可以参阅[数据处理适配器](https://www.yuque.com/rrqm/touchsocket/c260017413d76209ed86a27f267283af)，快速的解析数据。
