...
namespace RRQMSocket.FileTransfer
{
    ...
    public class FileClient : IFileClient, IDisposable
    {
        ...
        /// <exception cref="RRQMException"></exception>
        private void DownloadFile(UrlFileInfo  fileInfo, int waitTime = 5)
        {
            lock (locker)
            {
                if (!this.Online)
                ...
                byte[] datas = SerializeConvert.RRQMBinarySerialize(url, true);
                ...
                try
                {
                    ...
                    StartDownloadFile(waitResult.PBCollectionTemp.ToPBCollection(), url.Restart);
                }
                finally
                {
                    byteBlock.Dispose();
        ...
        public FileInfo RequestFileInfo(FileUrl url)
        {
            lock (locker)
            {
                ...
                try
                {
                    ...
                    if (returnData[0] == 1)
                    {
                        FileInfo fileInfo = SerializeConvert.RRQMBinaryDeserialize<FileInfo>(returnData, 1);
                        return fileInfo;
                    }
                    else if (returnData[0] == 2)
                    {
                        ...
                        try
                        {
                            ...
                            if (returnByteBlock == null || returnByteBlock.Buffer[0] == 2)
                            {
                                ...
                                if (reTryCount > 10)
                                {
                                    ...
                                    args.TransferType = TransferStatus.Download;
                                    args.Message = "下载文件错误";
                            ...
                            if (FileBaseTool.ReadFileBytes(this.uploadFileBlocks.FileInfo.FilePath, this.uploadPosition, byteBlock, 21, (int)submitLength))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (reTryCount > 10)
                                    {
                                        ...
                                        args.TransferType = TransferStatus.Upload;
                                        args.Message = "上传文件错误";
                                        TransferFileErrorMethod(this, args);
                                    }
                                }
