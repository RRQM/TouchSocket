...
- **推荐方式**，`TouchSocket` 框架内置的 `DataValidation` 验证
...
所以，`TouchSocket` 提供了新的验证引擎 `DataValidation`，在完全兼容 `Mvc` 内置验证的同时提供了大量常见验证、复杂验证、自定义验证等能力。

## 8.4 `DataValidation` 验证 🤗

`DataValidation` 是 `TouchSocket` 框架提供了全新的验证方式，完全兼容 `Mvc` 内置验证，并且赋予了超能。
...
<Tabs
  ...
  values={[
    ...
    { label: "TouchSocketAppService", value: "TouchSocketAppService" },
  ]}
>
...
```
  ...
  <TabItem value="TouchSocketAppService">
...
namespace TouchSocket.Application
{
    public class TestDto : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (Name.StartsWith("TouchSocket"))
            {
                yield return new ValidationResult(
                    "不能以 TouchSocket 开头"
                    , new[] { nameof(Name) }
...
`TouchSocket` 提供了 `TryValidate()` 和 `Validate()` 两个验证拓展方法，唯一的区别就是后者验证失败将自动抛出异常消息。

### 8.6.3 `ValidationTypes` 常见验证

`TouchSocket` 内置了很多常用类型的数据验证，包括：
...
// 自定义正则表达式验证
"TouchSocket".TryValidate("/^TouchSocket$"); // => true
```
...
`TouchSocket` 还提供了 `[DataValidation]` 特性方便在模型参数中使用 `ValidationTypes` 常见验证或自定义验证。
...
`TouchSocket` 框架提供了对象模型跳过验证特性 `[NonValidation]`，支持在 `控制器`，`动作方法`，`类` 中使用。
...
除了 `TouchSocket` 内置的验证类型以外，`TouchSocket` 还提供了非常灵活的自定义验证类型机制。
...
namespace TouchSocket.Application
{
    ...
    public enum MyValidationTypes
    {
        ...
        /// <summary>
        /// 以 TouchSocket 字符串开头，忽略大小写
        /// </summary>
        [ValidationItemMetadata(@"^(TouchSocket).*", "默认提示：必须以Fur字符串开头，忽略大小写", RegexOptions.IgnoreCase)]
        StartWithFurString
    }
}
```
...
`TouchSocket` 内置的 `ValidationTypes` 已有默认的失败消息：
...
namespace TouchSocket.Application
{
    ...
    public enum MyValidationMessageType
    {
        ...
        [ValidationMessage("必须以 TouchSocket 开头")]
        StartWithFurString
    }
}
```
...
除了贴 `[ValidationMessageType]` 特性外，`TouchSocket` 框架还提供了 `IValidationMessageTypeProvider` 方式查找验证消息类型，如下图所示：
...
```json showLineNumbers  {2-9} title="TouchSocket.Web.Entry/appsettings.json"
{
...
`TouchSocket` 内置的验证已经可以满足绝大多数校验情况，但是对于 `场景` 验证目前暂未支持。这里推荐集成 [`FluentValidation`](https://github.com/FluentValidation/FluentValidation) 第三方校验组件。
...
**在控制器中使用无需手动调用 `ModelState.IsValid` 进行判断，`TouchSocket` 会自动执行该操作。**
