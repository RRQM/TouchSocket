...
namespace TouchSocket.Smtp.Rpc
{
    ...
    /// Rpc传输类
    /// </summary>
    public sealed class SmtpRpcPackage : WaitRouterPackage
    {
        /// <summary>
        /// 反馈类型
        /// </summary>
        ...
        {
            if (option is SmtpInvokeOption smtpInvokeOption)
            {
        ...
        internal void ThrowStatus()
        {
            ...
            switch (this.Status.ToStatus())
            {
                case TouchSocketSmtpStatus.ClientNotFind:
                    {
                        throw new ClientNotFindException(TouchSocketSmtpStatus.ClientNotFind.GetDescription(this.TargetId));
                    }
                case TouchSocketSmtpStatus.UnknownError:
                case TouchSocketSmtpStatus.RpcMethodNotFind:
                case TouchSocketSmtpStatus.RpcMethodDisable:
                case TouchSocketSmtpStatus.RemoteRefuse:
                case TouchSocketSmtpStatus.RpcInvokeException:
                case TouchSocketSmtpStatus.RoutingNotAllowed:
                case TouchSocketSmtpStatus.Exception:
                default:
                    {
