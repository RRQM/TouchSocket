...
- 默认情况下必须放在启动层且主机启动时需通过 `.UseStartup<>` 进行注册，此问题在 `TouchSocket` 已解决 `AppStartup`
...
在 `TouchSocket 3.7.3+` 版本之后，借助 `Docker-Compose` 的设计理念，推出了全新的 `Component 组件化` 模式，通过组件化开发可以实现组件之间相互依赖，相互链接，还可以共享参数，你仅仅需要编写一个入口组件即可。
...
没错，这就是 TouchSocket 目前能够想到的最优解决方案。

## 3.2.4 `IComponent`

在 `TouchSocket 3.7.3+` 版本，新增了 `Components` 模块，该模块的根接口为 `IComponent`，含有两个派生接口 `IServiceComponent` 和 `IApplicationComponent`。
...
由于组件和组件之间存在依赖方式，甚至没有依赖关系但支持唤醒其他组件功能，所以 `TouchSocket` 提供了 `[DependsOn]` 特性。
...
`TouchSocket` 框架中已经处理了组件重复依赖问题，会自动生成好最佳的注册顺序并去除重复依赖注册问题。
