...
- 暂无<Highlight color="#25c2a0">beta</Highlight>
- 暂无<Highlight color="#F48916">beta.200</Highlight>
- [#I8DE1D](https://gitee.com/RRQM_Home/TouchSocket/issues/I8DE1D)<Highlight color="#1D98F5">beta.220</Highlight>
- 无需指南<Highlight color="#af38c7">beta.222</Highlight>
- 无需指南<Highlight color="#1fc2d4">beta.223</Highlight>
- 无需指南<Highlight color="#ee5e43">beta.224</Highlight>
- 无需指南<Highlight color="#9e9c0c">beta.225</Highlight>
...
- &nbsp;<Tag>优化</Tag> FileLogger支持指定不同目录。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 所有自定义插件必须在自身内，主动调用e.InvokeNext()时，才会调用下一个插件。不然会中断插件传递。同时e.Handled功能依然有效。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> Log项。LogType调整为LogLevel，并且不需要位运算。直接按日志等级输出。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>移除</Tag> DependencyProperty中，移除对类型的定义。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>移除</Tag> 所有组件的基础插件，强制用户插件必须继承PluginBase，然后实现需要的接口。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>移除</Tag> BytePool在创建ByteBlock时，移除EqualSize的设定，因为这会影响内存池的效率。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 修改所有委托为异步Task。 <Highlight color="#F48916">beta.200</Highlight>
- &nbsp;<Tag>调整</Tag> 修改所有Setup返回值为void。 <Highlight color="#1D98F5">beta.220</Highlight>
- &nbsp;<Tag>修复</Tag> Metadata在0个成员长度时，会被反序列化成null的bug。<Highlight color="#af38c7">beta.222</Highlight>
- &nbsp;<Tag>修复</Tag> PluginsManager在注册具有继承的插件时，会无法识别的bug。<Highlight color="#ee5e43">beta.224</Highlight>

#### 【TouchSokcet.Sokcets】

- &nbsp;<Tag>优化</Tag> IPHost支持从int、string直接隐式转换。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> TouchSocket所有“ID”属性，改名为“Id”。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> TouchSocket所有插件的执行顺序，移动至内部重写方法之后。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> TouchSocket所有`ResetID`改名为`ResetId`。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> UseCheckClear项，SetDuration调整名称为SetTick。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> UseCheckClear项，不仅可以适用服务器，客户端也适用。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> Config配置中，SetDataHandlingAdapter调整为SetTcpDataHandlingAdapter。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 适配器项，CustomDataHandlingAdapter中的Filter方法中，byteBlock参数使用in修饰。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 适配器项，DataHandlingAdapter改名为TcpDataHandlingAdapter。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 适配器项，DataAdapterTester改名为TcpDataAdapterTester。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>移除</Tag> UsePlugin的显式配置，当调用ConfigurePlugins时，会自动启用。<Highlight color="#25c2a0">beta</Highlight>

#### 【TouchSokcet.Http】

- &nbsp;<Tag>调整</Tag> WSCommandLinePlugin改名为WebSocketCommandLinePlugin。<Highlight color="#25c2a0">beta</Highlight>

#### 【TouchSokcet.Rpc】

- &nbsp;<Tag>移除</Tag> 整体功能迁移至TouchSokcet(Pro).Dmtp。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> [RpcActionFilter](./rpcactionfilter.mdx#33-规则)执行策略和顺序 <Highlight color="#1fc2d4">beta.223</Highlight>
- &nbsp;<Tag>新增</Tag> [RealityProxy](./generateproxyfromrealityproxy.mdx)透明代理方式。 <Highlight color="#9e9c0c">beta.225</Highlight>
- &nbsp;<Tag>新增</Tag> [DispatchProxy](./generateproxyfromdispatchproxy.mdx)添加OnBefore和OnAfter的AOP调用。 <Highlight color="#9e9c0c">beta.225</Highlight>

#### 【TouchSokcet(Pro).Dmtp】

- &nbsp;<Tag>调整</Tag> 原TouchRpc全系改名为Dmtp。例如：原TcpTouchRpcClient改名为TcpDmtpClient。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 原TouchRpc中InvokeOption，改名为DmtpInvokeOption。InvokeOption依然有效，但是在调用DmtpRpc时，则无法指定序列化方式。所以可能需要使用DmtpInvokeOption。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> 原TouchRpc中Invoke直接调用的方式，改为InvokeT。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>移除</Tag> **暂时**移除EventBus功能，后续可能考虑添加。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>新增</Tag> ws协议的TouchRpc服务端，增加HttpContext上下文获取。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>新增</Tag> 文件传输项，开放增加SetMaxSpeed功能。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>新增</Tag> DmtpRpc。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>新增</Tag> DmtpRpc组件在调用时，可以通过DmtpInvokeOption传入Metadata元数据。<Highlight color="#af38c7">beta.222</Highlight>

#### 【TouchSokcet.JsonRpc】

- &nbsp;<Tag>修复</Tag> JsonRpc使用内联数组调用[#I79OFZ](https://gitee.com/RRQM_Home/TouchSocket/issues/I79OFZ)。<Highlight color="#25c2a0">beta</Highlight>
- &nbsp;<Tag>调整</Tag> Rpc服务使用调用上下文时不需要再添加`IncludeCallContext`。<Highlight color="#25c2a0">beta</Highlight>
