...
|  插件方法| 功能 |
| --- | --- |
| IHttpDeletePlugin | 当收到Delete请求时 |
| IHttpGetPlugin | 当收到OnGet请求时 |
| IHttpPostPlugin | 当收到OnPost请求时 |
| IHttpPutPlugin | 当收到OnPut请求时 |
| IHttpPlugin | 当收到所有Http请求时。注意：此插件的执行在`IHttpGetPlugin`，`IHttpPostPlugin`等之前触发|
