...
所以，`TouchSocket` 实现了一套 `定位器` 功能，通过这个 `定位器` ，我们就能够通过 `依赖注入` 等多个方式定位到数据库上下文并初始化。
...
{
    public sealed class TouchSocketDbContextLocator : IDbContextLocator
    {
    }
}
```
...
在 `TouchSocket` 框架中已经提供了 `MasterDbContextLocator` 默认数据库上下文定位器，所以默认数据库上下文只需继承 `AppDbContext<TDbContext>` 即可。
