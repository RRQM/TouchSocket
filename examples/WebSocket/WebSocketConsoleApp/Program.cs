...
internal class Program
{
    ...
    private static async Task ConnectWith_wsheader()
    {
        ...
        #endregion WebSocket使用特定Header连接
    }
    ...
    private static async Task<HttpService> CreateHttpService2()
    {
        ...
        #endregion 创建WebSocket服务器 {11-16}
        ...
        #endregion WebSocket服务器遍历所有连接并发送数据
    ...
    private static async Task SendPingMessage()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            #endregion WebSocket发送Ping或者Pong消息
        }
    }
    ...
    public class MyServer : SingletonRpcServer
    {
        ...
        public async Task ConnectWS(IWebApiCallContext callContext)
        {
            ...
            {
                var result = await socketClient.SwitchProtocolToWebSocketAsync(true);
                if (!result.IsSuccess)
                {
    ...
    private class CustomWebSocketPlugin : PluginBase, IHttpPlugin
    {
        public async Task OnHttpRequest(IHttpSessionClient client, HttpContextEventArgs e)
        {
            if (e.Context.Request.IsUpgrade())
            {
                ...
                {
                    var result = await client.SwitchProtocolToWebSocketAsync(true);
                    if (!result.IsSuccess)
                    {
                        //切换协议失败
                        return;
