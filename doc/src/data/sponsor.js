const sponsors = [
  {
    title: "CRMEB 专注开源电商系统研发",
    picture: "img/crmeb.jpg",
    url: "http://github.crmeb.net/u/TouchSocket",
    top: true,
  },
  {
    title: "流之云 - 信息化、数字化服务提供商",
    picture: "img/tpflow.png",
    url: "https://www.gadmin8.com?from=TouchSocket",
    top: false,
  },
  {
    title: "CoreShop 移动端/小程序商城系统",
    picture: "img/coreshop.gif",
    url: "https://www.coreshop.cn?from=TouchSocket",
    top: false,
  },
  {
    title: "FirstUI 跨平台移动端组件库",
    picture: "img/firstui.jpeg",
    url: "https://www.firstui.cn?from=TouchSocket",
    top: false,
  },
  // {
  //   title: "JNPF 基于代码生成器的 .NET 框架",
  //   picture: "img/jnpfsoft.png",
  //   url: "https://dotnet.jnpfsoft.com/login?from=TouchSocket",
  //   top: false,
  // },
  {
    title: "Layui-Vue 开源前端 UI 框架",
    picture: "img/layui.png",
    url: "http://www.layui-vue.com?from=TouchSocket",
    top: false,
  },
  {
    title: "工作服定制T恤  一件起订来图定做",
    picture: "img/weishen.jpg",
    url: "https://eshan.tmall.com?from=TouchSocket",
    top: false,
  },
  {
    title: "ProcessOn：在线流程图思维导图工具",
    picture: "img/processon1.png",
    url: "https://www.processon.com?from=TouchSocket",
  },
  // {
  //   title: "Rust 语言圣经",
  //   picture: "img/rust.png",
  //   url: "https://course.rs/",
  // },
];

export default sponsors;
