...
public class TouchSocketService
{
    private readonly IRepository _repository;
    public TouchSocketService(IRepository repository)
    {
...
在 `TouchSocket` 新版本中，已经移除属性注入功能，建议使用构造函数或方法方式注入，也可以通过 `App.GetService<TService>` 方式注入。
...
`TouchSocket` 框架提供三个接口依赖分别对应不同的服务生存期：
...
`TouchSocket` 框架提供了非常灵活且方便的实现依赖注入的方式，只需要实例类继承对应生存期的接口即可，这里继承了 `ITransient`，也就表明了这是一个 `暂时/瞬时` 作用域实例类。该类就可以作为被注入对象，同时也能注入其他接口对象。
...
在 `TouchSocket 3.8.6+` 版本之后新增了 `INamedServiceProvider` 服务接口，可替代 `Func<string, ILifetime, object>` 方式：

```cs showLineNumbers {8,15-17,19-23}
using TouchSocket.Application.Services;
...
`TouchSocket` 框架提供 `[Injection]` 特性可以改变注册方式，同时还能配置 `AOP` 拦截。
...
默认情况下，`TouchSocket` 提供的注册方式可以满足大多数依赖注入的需求，如有特别注册需求，只需要在 `Startup` 中配置即可，如：
...
`TouchSocket` 框架中的 `AppDbContext` 数据库上下文还有 `ISqlDispatchProxy` 都是通过这种方式创建的。
...
`TouchSocket` 框架中，默认注册顺序是按照程序集扫描顺序进行注册，如果需要改变注册顺序，可通过 `[Injection(Order)]` 特性指定，`Order` 值越大，则越在最后注册。
...
在 `TouchSocket` 框架中，实现 `Aop` 非常简单，如：
...
`TouchSocket` 框架也提供了全局拦截的方式，只需要将 `IDispatchProxy` 修改为 `IGlobalDispatchProxy` 即可。
...
`TouchSocket` 框架未提供 `Proxy` 构造函数注入功能，但是提供了 `Services` 属性，如果需要解析服务，则可以通过以下方式：
...
为了方法快速创建服务作用域，`TouchSocket` 框架提供了 `Scoped` 静态类，如：
...
`TouchSocket` 框架中并未提供完全自定义依赖注入扫描的机制，但推荐一个非常优秀的 `.NET Core` 依赖注入拓展库：`Scrutor`，使用非常简单，主要通过 `FromAssemblyOf<>` 扫描程序集和 `AddClasses(o)` 进行筛选注册。
