...
using TouchSocket.ConfigurableOptions;
...
另外，`TouchSocket` 框架提供了非常灵活的注册选项服务的方法，只需要继承 **`IConfigurableOptions`** 接口即可，该接口位于 **`TouchSocket.ConfigurableOptions`** 命名空间下。
...
```cs showLineNumbers  {12} title="TouchSocket.Web.Core\FurWebCoreStartup.cs"
...
我们知道，选项实际上需要和配置文件特定键值挂钩，那 `TouchSocket` 是如何准确的找到配置文件中的键值的呢？
...
选项支持验证配置有效性，在 `TouchSocket` 框架中，通过 `services.AddConfigurableOptions<TOptions>()` 注册选项默认启用了验证支持。
...
在 `TouchSocket` 框架中，配置选项后期配置很简单，只需要继承 `IConfigurableOptions<TOptions>` 接口并实现 `PostConfigure(TOptions options)` 方法。
...
`TouchSocket` 框架提供了非常简单且灵活的方式监听选项更改，也就是 **`appsettings.json` 或 自定义配置文件发生任何更改都会触发处理方法**。
...
在 `TouchSocket` 底层使用的是 `ChangeToken.OnChange` 监听文件更改，但是此方式会导致 `OnListener` 触发两次，这并非是框架的 `bug`，而是 `.NET Core` 本身存在的问题，详见：[https://github.com/dotnet/aspnetcore/issues/2542](https://github.com/dotnet/aspnetcore/issues/2542)

所以，`TouchSocket` 框架也给出另一种解决方案可替代 `IConfigurableOptionsListener` 的方式，也就是通过局部注入 `IOptionsMonitor` 的方式，如：
...
以下内容仅限 `TouchSocket v3.4.3+` 版本使用。
