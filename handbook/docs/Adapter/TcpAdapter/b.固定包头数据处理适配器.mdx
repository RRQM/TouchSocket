...
Ushort包头算法：前2个字节，且为[默认端序（小端）](../../04、Core/4.3 大小端转换器.mdx)的排列，作为后续整个数据的长度，整个数据长度区间为\[0,65535]。
Int包头算法（默认配置）：前4个字节，且为[默认端序（小端）](../../04、Core/4.3 大小端转换器.mdx)排列，作为后续整个数据的长度，整个数据长度区间为\[0,2^31]。
