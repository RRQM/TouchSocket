...
通俗来说，就是使应用或系统支持多语言切换。`TouchSocket` 框架提供了完整支持多语言处理的服务。
...
`TouchSocket` 框架主要通过 `L` 静态类完成多语言转换，该静态类有以下属性和方法：
...
- `L.SetCurrentUICulture(区域码)`：运行时设置当前线程语言区域，立即有效，`TouchSocket 4.8.3.10+` 版本有效
- `L.GetCurrentUICulture()`： 获取当前线程 UI 区域性，`TouchSocket 4.8.3.10+` 版本有效
...
@using TouchSocket.Localization
...
以下内容仅限 `TouchSocket 4.3.7 +` 版本使用。

:::

正常情况下，我们都是通过 `L.Text["Hello"]` 方式输出 `Hello` 在不同的区域语言的翻译，但是会导致 `Hello` 硬编码字符串散落到处都是。

所以 `TouchSocket` 提供了 `SharedResource` 模式，只需要创建一个 `SharedResource.cs` 类（可以任何名字）并添加对应的属性即可，如：
...
在 `TouchSocket` 框架中，如果没找到对应的语言翻译文件，则自动显示字符串文本，如：
...
`TouchSocket` 框架也兼容 `.NET Core` 自带的依赖注入方式，如：
