...
`TouchSocket` 框架提供了非常方便且灵活的 `Swagger` 配置，无需增加额外学习成本。
...
```cs showLineNumbers  {13,21} title="TouchSocket.Web.Core\Startup.cs"
...
在 `TouchSocket` 框架中，默认 `规范化文档` 地址为 `/api` 目录，**支持自定义配置**。
...
`TouchSocket` 框架中默认分组名为 `Default`，**支持自定义配置**。
...
规范化文档默认扫描 `TouchSocket.Application`，`TouchSocket.Web.Core`，`TouchSocket.Web.Entry` 三个程序集`.xml` 注释文件，**支持自定义配置**。
...
多分组是一个系统中必备功能，我们可以将系统划分为多个模块，每个模块都独立的 `api` 配置。在 `TouchSocket` 框架中，实现多分组非常简单。如：
...
```json showLineNumbers  {2-17} title="TouchSocket.Web.Entry/appsettings.json"
{
  "SpecificationDocumentSettings": {
    "GroupOpenApiInfos": [
      {
...
`TouchSocket` 框架提供了可通过 `appsetting.json` 配置分组信息：

```json showLineNumbers  {3-20} title="TouchSocket.Web.Entry/appsettings.json"
{
  "SpecificationDocumentSettings": {
    "GroupOpenApiInfos": [
      {
        ...
        "Description": "这里是分组描述",
        "Version": "版本号",
        "TermsOfService": "https://TouchSocket.icu",
        "Contact": {
          "Name": "百小僧",
        ...
        "License": {
          "Name": "MIT",
          "Url": "https://gitee.com/rrqm_home/TouchSocket/blob/alpha/LICENSE"
        }
      }
    ]
  }
...
```json showLineNumbers  {2-4} title="TouchSocket.Web.Entry/appsettings.json"
{
  "SpecificationDocumentSettings": {
...
**新版本 `TouchSocket` 已经默认启用了 Bearer Token 授权配置，无需手动配置**，如需手动配置，可手动添加以下类似配置：

```json showLineNumbers  title="TouchSocket.Web.Entry/appsettings.json"
{
  "SpecificationDocumentSettings": {
...
`TouchSocket` 新版本提供了隐藏分组的 `Visible` 配置，设置为 `false` 之后该分组将不显示在规范化文档中，如：
...
`TouchSocket` 框架除了内置了不少配置以外，还提供了直接配置 `Swagger` Api 的参数，如：
...
```cs showLineNumbers {6,11}
 public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
 {
      // 如果使用 TouchSocket 4.4.8+ 版本可以 app.UseInject(options => {}) 了，无需指定 configure:
...
`TouchSocket` 框架默认只显示名称，如果需要自定义显示规则，只需要添加配置即可。
...
默认情况下，`TouchSocket` 框架会生成统一的分组模板，如：`swagger/{documentName}/swagger.json`，`{documentName}` 会在运行时替换为分组名，如需自定义，配置 `RouteTemplate` 即可：
...
在 `TouchSocket 2.9.0 + ` 版本已经支持了 `Swagger` 文档地址 `?culture=en-US` 参数多语言转发功能了，也就是 `Swagger` 地址带 `?culture=` 参数将自动添加到每一个请求的 `api` 地址中。
...
以下内容仅限 `TouchSocket v3.3.3+` 版本使用。
...
- `LoginInfo`：配置 `Swagger` 是否需要登录才能访问，`SpecificationLoginInfo` 类型，默认 `null`，**仅在 TouchSocket v3.3.3+` 有效**
...
using TouchSocket.SpecificationDocument;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using System.ComponentModel.DataAnnotations;

namespace TouchSocket.Web.Entry.Controllers;
...
在过去我们在接口定义的时候编写了完整的成员注释，然后在派生成员中又要重复写一次，实际上做了很大无用功，现在 `TouchSocket` 支持了注释继承了，同时 `Swagger` 中也能正确显示。
...
以下内容仅限 `TouchSocket v3.3.4+` 版本使用。
...
以下内容仅限 `TouchSocket v3.3.5+` 版本使用。
...
以下内容仅限 `TouchSocket v3.6.4+` 版本使用。
...
在 `TouchSocket` 最新版中，支持了完整的 `Markdown` 注册，如：
...
/// </summary>
/// <remarks>
/// # 先知 / TouchSocket ([探索版](https://gitee.com/rrqm_home/TouchSocket/tree/experimental/))
///
...
/// ## 安装 / Installation
///
/// - [Package Manager] (https://www.nuget.org/packages/TouchSocket)
///
/// ```powershell
/// Install-Package TouchSocket
/// ```
///
/// - [.NET CLI] (https://www.nuget.org/packages/TouchSocket)
///
/// ```powershell
...
/// ## 例子 / Examples
///
/// 我们在[主页](https://TouchSocket.baiqian.ltd)上有不少例子，这是让您入门的第一个：
///
/// We have several examples [on the website] (https://TouchSocket.baiqian.ltd). Here is the first one to get you started:
///
/// ```cs
/// Serve.Run();
///
...
/// ## 文档 / Documentation
///
/// 您可以在[主页] (https://TouchSocket.baiqian.ltd)或[备份主页](https://TouchSocket.icu)找到 TouchSocket 文档。
///
/// You can find the TouchSocket documentation[on the website](https://TouchSocket.baiqian.ltd) or [on the backup website](https://TouchSocket.icu).
///
/// ## 贡献 / Contributing
///
/// 该存储库的主要目的是继续发展 TouchSocket 核心，使其更快、更易于使用。 TouchSocket 的开发在[Gitee](https://gitee.com/rrqm_home/TouchSocket) 上公开进行，我们感谢社区贡献错误修复和改进。
///
/// 阅读[贡献指南] (https://TouchSocket.baiqian.ltd/docs/contribute)内容，了解如何参与改进 TouchSocket。
///
/// The main purpose of this repository is to continue evolving TouchSocket core, making it faster and easier to use.Development of TouchSocket happens in the open on[Gitee] (https://gitee.com/rrqm_home/TouchSocket), and we are grateful to the community for contributing bugfixes and improvements.
///
/// Read[contribution documents] (https://TouchSocket.baiqian.ltd/docs/contribute) to learn how you can take part in improving TouchSocket.
///
/// ## 许可证 / License
///
/// TouchSocket 采用[MIT](https://gitee.com/rrqm_home/TouchSocket/blob/v4/LICENSE) 开源许可证。
///
/// TouchSocket uses the[MIT] (https://gitee.com/rrqm_home/TouchSocket/blob/v4/LICENSE) open source license.
///
/// ```
/// Copyright(c) 2020-2023 百小僧, Baiqian Co., Ltd.
/// TouchSocket is licensed under Mulan PSL v2.
/// You can use this software according to the terms andconditions of the Mulan PSL v2.
/// You may obtain a copy of Mulan PSL v2 at:
///             https://gitee.com/rrqm_home/TouchSocket/blob/v4/LICENSE
...
在 `TouchSocket` 框架中，集成第三方 `SwaggerUI` 非常容易，比如集成 `IGeekFan.AspNetCore.Knife4jUI`：
...
除了上述例子外，`TouchSocket` 提供了一些配置选项，如：
...
- `DocExpansionState`：文档显示方式，`DocExpansion`，默认 `List`，取值：
  ...
  - `Full`：完全展开
  - `None`：列表式（不展开子类）
- `XmlComments`：程序集注释描述文件名（可带 `.xml`，`string`，默认 `TouchSocket.Application, TouchSocket.Web.Entry, TouchSocket.Web.Core`
...
- `ServerDir`：配置 `IIS` 添加 `Application` 部署名，`string` 类型，默认空，**仅在 TouchSocket v3.2.0+` 有效**
- `LoginInfo`：配置 `Swagger` 是否需要登录才能访问，`SpecificationLoginInfo` 类型，默认 `null`，**仅在 TouchSocket v3.3.3+` 有效**
  - `Enabled`：是否启用登录授权，默认 `false`
  - `CheckUrl`：检查登录状态的 `Url` 地址，**该地址必须是 `POST` 请求**，已授权返回 `200`，否则返回 `401`
  - `SubmitUrl`：提交登录的 `Url` 地址，**该地址必须是 `POST` 请求且只有一个 `SpecificationAuth` 类型参数**，成功登录返回 `200`，否则返回 `401`，支持相对地址，以 `/` 开头
- `EnableAllGroups`：启用 `Swagger` 总分组功能，自动将所有分组的接口合并到 `All Groups` 中，`bool` 类型，默认 `false`，**仅在 TouchSocket v3.3.4+` 有效**
...
`TouchSocket` 框架提供 `规范化结果` 功能，可以通过实现 `IUnifyResultProvider` 提供器实现统一规范化返回值定制，如：
    ...
    public class YourRESTfulResultProvider  : IUnifyResultProvider
    {
        ...
        public IActionResult OnException(ExceptionContext context, ExceptionMetadata metadata)
        {
            return new JsonResult(YourRESTfulResult(metadata.StatusCode, data: metadata.Data, errors: metadata.Errors)
                , UnifyContext.GetSerializerSettings(context)); // 当前行仅限 TouchSocket 4.6.6+ 使用
        }
