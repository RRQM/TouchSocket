...
`TouchSocket` 推荐采用多层项目设计架构，每一个项目层的依赖分别是：

- `YourName.Application`：添加 `YourName.Core` 引用
- **`YourName.Core`：添加 `TouchSocket` 引用**，**SqlSugar 版本添加 `TouchSocket.Pure`** 🎗
...
## 2.3.2 集成 `TouchSocket` 功能

项目搭建好之后，集成 `TouchSocket` 框架只需要在 `Program.cs` 中 添加 `Inject()` 方法即可：
