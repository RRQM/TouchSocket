...
`TouchSocket` 通过独创的 `数据库上下文定位器` 实现多数据库灵活操作切换。只需要为每一种数据库绑定唯一的数据库上下文定位器即可。

以下是 `TouchSocket` 支持多数据库操作的实例：
...
可查阅相关 `Issue`：[https://gitee.com/rrqm_home/TouchSocket/issues/I3RSCO](https://gitee.com/rrqm_home/TouchSocket/issues/I3RSCO)
