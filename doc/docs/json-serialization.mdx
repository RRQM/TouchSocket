...
- `System.Text.Json`：`.NET Core` 内置 `JSON` 序列化库，也是 `TouchSocket` 框架默认实现
...
`TouchSocket` 框架为了解决多种序列化工具配置和用法上的差异问题，抽象出了 `IJsonSerializerProvider` 接口。

## 23.3 `IJsonSerializerProvider` 接口

`TouchSocket` 框架提供了 `IJsonSerializerProvider` 接口规范，同时**要求实现该接口的实体都必须采用单例模式**，该接口定义代码如下：

```cs showLineNumbers
namespace TouchSocket.JsonSerialization
{
    /// <summary>
    /// Json 序列化提供器
    /// </summary>
...
`TouchSocket` 框架提供了两种方式获取 `IJsonSerializerProvider` 实例：
...
using TouchSocket.JsonSerialization;
...
{
    var json = "{\"Id\":1,\"Name\":\"TouchSocket\"}";
...
`TouchSocket` 框架不推荐一个框架中有多种序列化实现类，也就是说使用 `System.Text.Json` 就不要使用 `Newtonsoft.Json`，反之亦然。
...
正如上文所说，`TouchSocket` 默认的 `IJsonSerializerProvider` 实现方式是 `System.Text.Json` 库，如需替换为 `Newtonsoft.Json`，只需以下步骤即可：

:::tip 无需安装

在 `TouchSocket 4.6.5+` 版本已经内置了 `Microsoft.AspNetCore.Mvc.NewtonsoftJson` 拓展包，也就是直接在 `Startup.cs` 中注册即可。
...
在 `.NET6+` 添加了 `DateOnly` 和 `TimeOnly` 类型，`TouchSocket 4.7.9+` 提供了支持。
...
由于默认 `TouchSocket` 采用 `System.Text.Json` 进行序列化，但是不支持复杂类型，如 `DataTable`、`DataSet`、`Tuple` 、`JArray`，`JObject`等类型，所以需要更换成 `NewtonsoftJson` 即可，见 [JSON 序列化 - 23.5.1 自定义序列化提供器](./json-serialization#2351-自定义序列化提供器)
