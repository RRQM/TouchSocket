...
namespace TouchSocket.Application.Persons
{
...
`TouchSocket` 默认不支持 动态 `Sql` 查询功能，不过可以通过第三方实现：

在 `TouchSocket` 项目层安装 `System.Linq.Dynamic.Core` 包 [https://github.com/zzzprojects/System.Linq.Dynamic.Core](https://github.com/zzzprojects/System.Linq.Dynamic.Core)
...
var list = repository.AsQueryable()
                     .Where("Name.Contains(@0)","TouchSocket")
                     .ToList();
...
以下内容在 `TouchSocket 2.13 +` 版本中已移除。

:::

`TouchSocket` 框架还提供了时态查询功能，可以查询特定时间的数据，如：

```cs showLineNumbers 
var result = rep.Entities
    .AsTemporalOf(DateTime.UtcNow.AddDays(-1))
    .Include(i=> i.Company)
    .FirstOrDefault(i => i.Name == "TouchSocket");
```
