...
`TouchSocket` 框架提供了非常灵活的友好异常处理方式。
...
namespace TouchSocket.FriendlyException
{
...
在 `TouchSocket` 框架中，提供了非常灵活且规范化的友好异常处理方式，通过这个方式可以方便管理异常状态码、异常信息及异常本地化。
...
`TouchSocket` 框架提供了 `[ErrorCodeType]` 特性和 `IErrorCodeTypeProvider` 提供器接口来提供异常信息扫描，这里用的是 `[ErrorCodeType]` 特性类。

:::

### 7.6.2 关于 `[ErrorCodeItemMetadata]`

`TouchSocket` 框架提供了`[ErrorCodeItemMetadata]` 特性用来标识**枚举字段**异常元数据，该特性支持传入 `消息内容` 和 `格式化参数`。最终会使用 `String.Format(消息内容，格式化参数)` 进行格式化。
...
在 `TouchSocket` 框架中，还提供了 `IErrorCodeTypeProvider` 异常消息提供器接口，方便在不能贴 `[ErrorCodeType]` 特性情况下使用：
...
`TouchSocket` 框架还提供了非常灵活的配置文件配置异常，通过这种方式可以实现异常信息后期配置，也就是无需在开发阶段预先定义。

```json showLineNumbers  {2-8} title="TouchSocket.Web.Entry/appsettings.json"
{
...
`TouchSocket` 框架提供了 `[IfException]` 特性可以**覆盖默认消息配置**。也就是覆盖 `异常消息类型` 和 `appsettings.json` 中的配置。
...
以下内容仅限 `TouchSocket 3.6.1 +` 版本使用。

:::

`TouchSocket` 在该版本之后内置了 `BadPageResult` 错误结果类型，该类型派生自 `IActionResult`，如需返回只需要在 `Action` 中返回即可。
