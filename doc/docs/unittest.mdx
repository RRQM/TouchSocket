...
**在本章节，`TouchSocket` 框架使用 `xUnit` 库进行单元测试。**
...
## 36.1.6 集成 `TouchSocket` 强大功能

`TouchSocket` 是跨平台、跨项目的开发框架，支持任意项目类型，包括单元测试项目。

### 36.1.6.1 安装 `TouchSocket.Xunit` 包

:::note `TouchSocket` 纯净版

如果使用的是 `TouchSocket.Pure` 则安装 `TouchSocket.Pure.Xunit` 这个拓展包。

:::

打开 `Nuget` 程序包控制台，安装 `TouchSocket.Xunit` 包

<img src={useBaseUrl("img/ut6.png")} />

:::important 特别注意

`TouchSocket.Xunit` 已经包含 `TouchSocket` 无需再次安装 `TouchSocket`。
...
### 36.1.6.3 使用 `TouchSocket` 完整功能

`TouchSocket` 是跨平台、跨项目的开发框架，下面在单元测试中演示 `远程请求` 并请求 `https://www.baidu.com` 数据，并测试是否请求成功。
...
很神奇吧！`TouchSocket` 支持任何项目类型，任何平台使用。
...
有些时候，我们需要测试某接口，或者进行依赖注入方式解析服务并调用，`TouchSocket.Xunit` 提供完整的构造函数注入。
...
namespace TestProject1
{
    public class UnitTest1
    {
        ...
        {
            Output.WriteLine("哈哈哈哈，我是 TouchSocket");
            Assert.NotEqual("TouchSocket", "Fur");
        }
    }
}
```
...
`TouchSocket` 会在创建单元测试实例时创建一个 `IServiceScope` 对象，等该实例所有测试案例执行完毕自动调用 `Dispose`，编写测试的开发者无需关注。
...
### 36.1.9.4 集成 `TouchSocket.Xunit` 拓展

`Web` 集成测试支持完整的 `TouchSocket` 特性，参考上面单元测试集成 `TouchSocket` 章节。
