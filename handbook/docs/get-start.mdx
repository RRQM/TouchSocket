...
使用 `TouchSocket` 之前先确保安装了最新的 `.NET 5 SDK` 并升级 `Visual Studio 2019` 至最新版。
...
`TouchSocket` 已经内置了 `Swagger` 规范化库，所以创建时**无需勾选** `Enable OpenAPI support` 选项。否则提示版本不一致产生冲突。

:::

## 2.1.2 添加 `TouchSocket` 依赖包

<img src={useBaseUrl("img/rm4.png")} />

## 2.1.3 `TouchSocket` 基本配置
