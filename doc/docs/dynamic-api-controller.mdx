...
namespace TouchSocket.Web.Entry.Controllers
{
    ...
    public class WebApiController : ControllerBase
    {
        ...
        {
            return Content(nameof(TouchSocket));
        }
    }
}
```
...
针对以上 `ASP.NET Core` 提供的 `WebAPI` 必须遵循的约定和不可避免的缺点，`TouchSocket` 框架创造出一种更加灵活创建 `WebAPI` 控制器的方式。
...
```cs showLineNumbers  {11} title="TouchSocket.Web.Core\FurWebCoreStartup.cs"
...
创建一个 `TouchSocketAppService` 类继承 `IDynamicApiController` 接口 或 贴 `[DynamicApiController]` 特性，并在这个类中编写一个 `Get` 方法。
...
namespace TouchSocket.Application
{
    public class TouchSocketAppService : IDynamicApiController
    {
        public string Get()
        {
            return $"Hello {nameof(TouchSocket)}";
        }
    }
}
```
...
namespace TouchSocket.Application
{
    ...
    public class TouchSocketAppService
    {
        public string Get()
        {
            return $"Hello {nameof(TouchSocket)}";
...
`TouchSocket` 框架会在应用启动时注册 `DynamicApiControllerFeatureProvider` 控制器特性提供器，该提供器继承自 `ControllerFeatureProvider` 类。

接着重写 `bool IsController(TypeInfo typeInfo)` 方法，用来标识控制器类型。在 `TouchSocket` 框架中，**继承自 `ControllerBase` 类或 `IDynamicApiController` 接口或 `[DynamicApiController]` 特性都会被标记为控制器类型。**

### 5.1.6.2 应用模型转换器

`TouchSocket` 框架同时在应用启动时注册 `DynamicApiControllerApplicationModelConvention` 应用模型转换器，该转换器继承自 `IApplicationModelConvention` 接口。
...
- 生成控制器名称默认去除以 `AppServices，AppService，ApiController，Controller，Services，Service` 作为前后缀的字符串。见第一个例子中的 `TouchSocketAppService -> TouchSocket` **支持自定义配置**
- 控制器名称带 `V[0-9_]` 结尾的，会自动生成控制器版本号，如 `TouchSocketAppServiceV2 -> TouchSocket@2`，`TouchSocketAppServiceV1_1_0 -> TouchSocket@1.1.0`。**支持版本分隔符配置**
...
`TouchSocket` 框架提供了非常方便的自定义参数位置的特性 `[ApiSeat]`，通过 `[ApiSeat]` 可配置参数位置，支持以下四种位置：
...
`TouchSocket` 框架提供了非常灵活的各种路由组合方式，支持一对多，多对多路由组合：
...
namespace TouchSocket.Application
{
    public class TouchSocketAppServiceV1 : IDynamicApiController
    {
        public string Get()
        {
    ...
    public class TouchSocketAppServiceV1_2 : IDynamicApiController
    {
        public string Get()
        {
            return nameof(TouchSocket);
    ...
    public class TouchSocketAppServiceV1_2_1 : IDynamicApiController
    {
        public string Get()
        {
            return nameof(TouchSocket);
...
**`V[0-9_]`** 结尾的命名自动解析成版本号，如 **`TouchSocketAppServiceV2 -> TouchSocket@2`**。
...
生成的路由为：`https://xxx.com?id=1&name=TouchSocket`
...
`TouchSocket` 框架提供了多种参数特性配置参数绑定规则：
...
在 `TouchSocket` 框架中，在没有配置 `[HttpMethod]` 特性的情况下，会自动根据方法名第一个参数进行分析，并生成对应的 `[HttpMethod]` 特性，规则如下：
...
在 `TouchSocket v2.8.6` 版本中实现了 `[FromRoute]` 参数非必填功能，支持以下几种方式：
...
在 `TouchSocket v2.8.6` 版本中，添加了 `[RouteConstraint]` 特性，可配置路由约束，如：`[RouteConstraint(":min(10)")]`
...
除了上述 `ASP.NET Core` 提供的配置外，`TouchSocket` 框架还提供了非常强大且灵活的 `[ApiDescriptionSettings]` 特性。
...
namespace TouchSocket.Application
{
    ...
    public class MyTouchSocketAppService : IDynamicApiController
    {
...
`TouchSocket` 还提供动态 `WebAPI` 接口一些全局配置选项，如：
...
默认情况下，`TouchSocket` 动态 `WebAPI` 接口不对 `ControllerBase` 类型进行任何处理。当然，我们也可以手动启用 `ControllerBase` 支持。
