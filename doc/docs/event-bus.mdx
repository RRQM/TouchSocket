...
**在 `TouchSocket v2.20+` 版本采用 [Jaina](https://gitee.com/dotnetchina/Jaina) 事件总线替换原有的 `EventBus`**，[查看旧文档](/docs/event-bus-old)

:::

:::important 版本说明

以下内容仅限 `TouchSocket 2.20.0 +` 版本使用。
...
public class ToDoEventSubscriber : IEventSubscriber
{
    ...
    // 支持多种异常重试配置，TouchSocket 4.2.10+ 版本支持
    ...
    // 支持简单 Order 编排，TouchSocket 4.8.0+ 版本支持
...
在 `TouchSocket` 中可以不用通过 `builder.AddSubscriber<T>()` 方式一一注册，只需要实现 `ISingleton` 接口即可，如：
...
`TouchSocket` 使用 `IEventSource` 作为消息载体，任何实现该接口的类都可以充当消息载体。
...
services.AddEventBus(builder =>
{
    ...
    // TouchSocket 4.7.6 版本新增 ReplaceStorerOrFallback ，可在自定义事件源存储器初始失败时回退到默认值
...
`TouchSocket` 默认内置基于 `Channel` 的事件发布者 `ChannelEventPublisher`。
...
`TouchSocket` 提供了 `IEventHandlerMonitor` 监视器接口，实现该接口可以监视所有订阅事件，包括 `执行之前、执行之后，执行异常，共享上下文数据`。
...
`TouchSocket` 提供了 `IEventHandlerExecutor` 执行器接口，可以让开发者自定义事件处理函数执行策略，如 `超时控制，失败重试、熔断等等`。
...
在 `TouchSocket` 中， `Event Bus` 所有服务均注册为单例，如需使用作用域服务（单例服务可直接注入），可通过依赖注入 `IServiceProvider` 实例并通过 `CreateScope()` 创建一个作用域，如：
...
以下内容仅限 `TouchSocket 4.2.10 +` 版本使用。

:::

在该版本中，`TouchSocket` 提供了 `IEventBusFactory` 工厂服务，可在运行时动态新增或删除订阅，如：
...
以下内容仅限 `TouchSocket 4.3.3 +` 版本使用。

:::

在 `TouchSocket 4.3.3` 版本新增了 `MessageCenter` 静态类，可在任何地方发送事件消息或订阅消息。
