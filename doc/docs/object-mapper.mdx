...
`TouchSocket` 框架提供了 `Mapster` 的拓展 `TouchSocket.Extras.ObjectMapper.Mapster`，如需使用第三方如 `AutoMapper` 则无需安装此拓展。
...
所以，实现自动映射赋值和支持特殊配置的需求就有了。目前 `C#` 平台有两个优秀的对象映射工具：`Mapster` 和 `AutoMapper`。**在 `TouchSocket` 框架中，推荐使用 [Mapster](https://github.com/MapsterMapper/Mapster)，[Mapster](https://github.com/MapsterMapper/Mapster) 是一款极易使用且超高性能的对象映射框架。**
...
在 `TouchSocket.Core` 层安装 `TouchSocket.Extras.ObjectMapper.Mapster` 拓展包，无需手动调用，`TouchSocket` 会自动加载并调用。
...
该映射文件 `Mapper.cs` 可以放在任何项目或文件夹中，`TouchSocket` 会在程序启动的时候自动扫描并注入配置。
...
`TouchSocket` 提供全局默认映射配置选项 `TypeAdapterConfig.GlobalSettings.Default`，可在 `Startup` 中配置即可，如：
