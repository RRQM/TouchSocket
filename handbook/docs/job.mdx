...
**在 `TouchSocket 4.8.0+` 版本采用 [Sundial](https://gitee.com/dotnetchina/Sundial) 定时任务替换原有的 `TaskScheduler`**，[查看旧文档](/docs/job-old)

:::

:::important 版本说明

以下内容仅限 `TouchSocket 4.8.0 +` 版本使用。
...
以下内容仅限 `TouchSocket 4.8.3.7 +` 版本使用。
...
**但这样的 `范式代码` 几乎每一个 `IJob` 实现类都可能输出**，所以在 `TouchSocket 4.8.3.7+` 版本提供了更便捷的配置，无需每一个 `IJob` 编写 `_logger.LogInformation($"{context}");`。
...
info: 2022-12-14 11:56:12.3963326 +08:00 星期三 L TouchSocket.Application.MyJob[0] #4
...
以下内容仅限 `TouchSocket 4.8.4.3 +` 版本使用。

:::

在 `TouchSocket 4.8.4.3+` 版本新增了 `GetTimelines()` 方法，可获取内存中作业触发器最近运行的 `10` 条记录，如：
...
以下内容仅限 `TouchSocket 4.8.3.3 +` 版本使用。

:::

通常情况下我们通过 `_schedulerFactory.GetJob("jobId")` 获取到作业之后，然后对这个作业进行操作，**但操作之后这个对象并不能同步更改，需要反复调用 `GetJob` 方法。**。

所以在 `TouchSocket 4.8.3.3+` 版本之后，**`IScheduler` 任何操作都将自动调用 `Reload()` 方法刷新变量**。
...
`TouchSocket` 框架提供了 `Serve.Run()` 方式支持跨平台使用，还能支持注册更多服务，如：
...
在 `TouchSocket 4.8.4+` 版本内置了一个嵌入的定时任务看板 `UI`，只需要在 `Startup.cs` 中启用即可，如：
...
**注意：当前 `TouchSocket 4.8.4+` 版本暂未提供任何自定义功能，包括 `/schedule` 地址配置。**

前端源码地址：[https://gitee.com/rrqm_home/TouchSocket/tree/v4/clients/schedule-dashboard](https://gitee.com/rrqm_home/TouchSocket/tree/v4/clients/schedule-dashboard)
