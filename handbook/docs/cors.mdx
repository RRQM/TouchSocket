...
| --------------------------- | --------------------------------- | -------- | ------------------------------ |
| http://www.TouchSocket.icu/      | http://www.TouchSocket.icu/index.html  | 否       | 同源（协议、域名、端口号相同） |
| http://www.TouchSocket.icu/      | https://www.TouchSocket.icu/index.html | 跨域     | 协议不同（http/https）         |
| http://www.TouchSocket.icu/      | http://www.baidu.com/             | 跨域     | 主域名不同（chinadot/baidu）   |
| http://www.TouchSocket.icu/      | http://blog.TouchSocket.icu/           | 跨域     | 子域名不同（www/blog）         |
| http://www.TouchSocket.icu:8080/ | http://www.TouchSocket.icu:7001/       | 跨域     | 端口号不同（8080/7001）        |
...
默认情况下，`TouchSocket` 允许所有域名来源访问，也就是无需配置任何来源域名，另外前端也需要设置请求参数：`withCredentials:false`
...
**在 `TouchSocket 4.1.4+` 版本已修正 `SignalR` 跨域问题，只需要启用 `SignalRSupport` 配置即可**，如：
