...
在阅读下面文档之前，必须把 `TouchSocket` 源码文件夹下的 `tools/cli.ps1` 文件拷贝到本地中（**不能直接复制源码自行创建，会有编码异常问题**）。而且 `Fur.Web.Entry` 层需要安装 `Microsoft.EntityFrameworkCore.Tools` 包。
...
`TouchSocket` 提供两种主要方法来 **保持实体模型和数据库架构同步**。
...
目前 `TouchSocket Tools` 生成工具**默认不支持任何数据库**生成，所以如需生成特定数据库的代码，只需要在 `TouchSocket.EntityFrameworkCore.Core` 安装对应的数据库包即可：
...
注意：开始之前先把 `TouchSocket.Web.Entry` 设为启动项目。

<img src={useBaseUrl("img/dbfirst1.png")} />

### 9.19.2.2 切换默认项目

将 `程序包管理控制台` 默认项目设置为 `TouchSocket.Core`，如果您是其他名字，则切换对应即可。
...
```shell showLineNumbers
TouchSocket Tools v1.0.0 请键入操作类型：[G] 界面操作，[任意字符] 命令行操作
TouchSocket Tools v1.0.0 您的输入是:
```
...
这时，`TouchSocket Tools` 会自动查找所有数据库配置连接字符串的 `.json` 文件：
...
```json showLineNumbers  {12-13}
{
  ...
  "AllowedHosts": "*",
  "ConnectionStrings": {
    "DbConnectionString": "Server=localhost;Database=TouchSocket;User=sa;Password=000000;MultipleActiveResultSets=True;",
    "Sqlite3ConnectionString": "Data Source=./TouchSocket.db"
  }
}
```
...
点击 `立即生成` 按钮后，会弹出实体保存选择目录资源管理器，默认实体只能保存在 `TouchSocket.Core` 层：
...
//
// -----------------------------------------------------------------------------
TouchSocket Tools v1.0.0 启动中......
TouchSocket Tools v1.0.0 启动成功！
TouchSocket Tools v1.0.0 请键入操作类型：[G] 界面操作，[任意字符] 命令行操作
TouchSocket Tools v1.0.0 您的输入是: G
TouchSocket Tools v1.0.0 正在加载数据库表和视图......
TouchSocket Tools v1.0.0 加载成功！
TouchSocket Tools v1.0.0 正在编译解决方案代码......
Build started...
Build succeeded.
...
TouchSocket Tools v1.0.0 编译成功！
TouchSocket Tools v1.0.0 开始生成实体文件......
TouchSocket Tools v1.0.0 正在生成 City.cs 实体代码......
TouchSocket Tools v1.0.0 成功生成 City.cs 实体代码
// -----------------------------------------------------------------------------
// 以下代码由 TouchSocket Tools v1.0.0 生成
// -----------------------------------------------------------------------------
...
TouchSocket Tools v1.0.0 正在生成 PersonDetail.cs 实体代码......
TouchSocket Tools v1.0.0 成功生成 PersonDetail.cs 实体代码
// -----------------------------------------------------------------------------
// 以下代码由 TouchSocket Tools v1.0.0 生成
// -----------------------------------------------------------------------------
...
TouchSocket Tools v1.0.0 正在生成 Post.cs 实体代码......
TouchSocket Tools v1.0.0 成功生成 Post.cs 实体代码
// -----------------------------------------------------------------------------
// 以下代码由 TouchSocket Tools v1.0.0 生成
// -----------------------------------------------------------------------------
...
TouchSocket Tools v1.0.0 正在生成 VPerson.cs 实体代码......
TouchSocket Tools v1.0.0 成功生成 VPerson.cs 实体代码
// -----------------------------------------------------------------------------
// 以下代码由 TouchSocket Tools v1.0.0 生成
// -----------------------------------------------------------------------------
...
TouchSocket Tools v1.0.0 全部实体生成成功！
PM>
```

## 9.19.4 命令参数配置

`TouchSocket Tools Cli` 支持多个参数配置，使用方法只需要在命令后面添加即可，如：
...
- `-Context`：配置数据库上下文，默认 `TouchSocketDbContext`，如果有多个数据库上下文，则此参数必须配置
- `-ConnectionName`：配置数据库连接字符串，对应 `appsetting.json` 中的 `ConnectionStrings` 定义的 `Key`
- `-OutputDir`：生成实体代码输出目录，默认为：`./TouchSocket.Core/Entities/`
...
- `-EntryProject`：Web 启用项目层名，默认 `TouchSocket.Web.Entry`
- `-CoreProject`：实体项目层名，默认 `TouchSocket.Core`
- `-DbContextLocators`：多数据库上下文定位器，默认 `MasterDbContextLocator`，支持多个，如：`MasterDbContextLocator,MySqlDbContextLocator`
- `-Product`：解决方案默认前缀，如 `TouchSocket`
