...
在 `TouchSocket` 中实现视图的操作非常简单，只需要创建视图模型，并继承 `EntityNotKey` 基类即可。代码如下：
...
{
    public class TouchSocketService : IDynamicApiController
    {
        private readonly IReadableRepository<V_Person> _readableRepository;

        public TouchSocketService(IRepository<V_Person> repository)
        {
            // 初始化只读仓储
