...
**在 `TouchSocket 4.8.0+` 版本采用 [Sundial](https://gitee.com/dotnetchina/Sundial) 定时任务替换原有的 `TaskScheduler`**，**😶[查看新文档](/docs/job)**
...
为了方便定义 `Cron` 表达式，`TouchSocket` 框架也提供了非常方便的占位符实现常用的时间格式：
...
默认情况下，`TouchSocket` 框架未启用对 `秒` 的支持，如需开启，则设置 `cronFormat: CronFormat.IncludeSeconds` 即可。默认值是 `cronFormat: CronFormat.Standard`
...
除了上面的 `SpareTime` 静态类方式，`TouchSocket` 框架还提供了 `ISpareTimeWorker` 方式，使用该方式非常简单，只需要自定义一个**公开且非抽象非静态**类并实现 `ISpareTimeWorker` 即可。
...
`TouchSocket` 框架提供了任务两种执行方式 `并行` 和 `串行`：
...
有些时候我们可能在执行任务过程中出现异常，`TouchSocket` 也提供了属性判断是否有异常和异常信息，方便记录到日志中，如：
...
在 `TouchSocket v2.18+` 版本之后新增了定时任务监听器 `ISpareTimeListener`，通过监听器可以实现所有定时任务的状态。如，创建一个 `单例` 的监听器 `SpareTimeListener`：

```cs showLineNumbers  {8,15}
using TouchSocket.DependencyInjection;
using TouchSocket.TaskScheduler;
using System;
