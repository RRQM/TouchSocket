...
{
    public class TestModbusUdpMaster
    {
        ...
        public void ReadDiscreteInputs()
        {
            ...
            var response = client.ReadDiscreteInputs(1,1000, 10);
        ...
        {
            var client = this.GetMaster();
            var response = client.ReadInputRegisters(1,1000, 10);
        ...
        public void ReadWriteCoilsShouldBeOk()
        {
            ...
            //写单个线圈
            client.WriteSingleCoil(1,1000, true);
            client.WriteSingleCoil(1,1001, false);

            //写多个线圈
            client.WriteMultipleCoils(1,1002, new bool[] { true, false, true });

            //读取线圈
            var values = client.ReadCoils(1,1000, 5);
        ...
        public void ReadWriteHoldingRegisters()
        {
            ...
            //写入单个寄存器
            client.WriteSingleRegister(1,1000, 1);//默认ABCD端序
            client.WriteSingleRegister(1,1001, 1000);//默认ABCD端序
            ...
            using (var valueByteBlock = new ValueByteBlock(1024))
            {
                ...
                //写入多个寄存器
                client.WriteMultipleRegisters(1,1002, valueByteBlock.ToArray());
            }

            //读取寄存器
            var response = client.ReadHoldingRegisters(1,1000, 1 + 1 + 1 + 1 + 2 + 4);
