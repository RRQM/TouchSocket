...
public class MyService : TcpService<MySessionClient>
{
    ...
    protected override async Task OnConnecting(MySessionClient SessionClient, ConnectingEventArgs e)
    {
        //此处逻辑会多线程处理。
        ...
        await base.OnConnecting(SessionClient, e);
    }
}
```
...
```csharp showLineNumbers
TcpService<MySessionClient> service = new TcpService<MySessionClient>();
...
### 9.2 重写SessionClient处理

正如6.2所示，可以直接在MySessionClient的重写**HandleReceivedData**中直接处理数据。
...
```csharp showLineNumbers
var SessionClients = service.GetClients();
```

:::caution 注意

由于SessionClient的生命周期是由框架控制的，所以最好尽量不要直接引用该实例，可以引用SessionClient.Id，然后再通过服务器查找。
...
```csharp showLineNumbers
var ids = service.GetIds();
if (service.TryGetClient(ids.First(), out SessionClient SessionClient))
{
}
```
