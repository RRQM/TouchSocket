...
{
    public async Task OnWebSocketHandshaking(TSocketClient client, HttpContextEventArgs e)
    {
        client.Logger.Info("WebSocket正在连接");
        await e.InvokeNext();
    }

    public async Task OnWebSocketHandshaked(TSocketClient client, HttpContextEventArgs e)
    {
        client.Logger.Info("WebSocket成功连接");
        await e.InvokeNext();
    }

    public async Task OnWebSocketReceived(TSocketClient client, WSDataFrameEventArgs e)
    {
