...
默认情况下，`TouchSocket` 框架会自动扫描根目录下的 `*.json` 和 `*.config.xml` 文件载入配置中，如需忽略个别文件，需在 `appsettings.json` 配置文件根节点下配置 `IgnoreConfigurationFiles` 节点即可，`string[]` 类型，如：
