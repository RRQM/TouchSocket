---
title: 2.8 心跳设计
url: https://www.yuque.com/rrqm/touchsocket/szxe2w7qepcl8le4
---
...
其实TCP中已经为我们实现了一个[内置心跳机制（SetKeepAliveValue）](<2.3 创建TcpClient.md>)。但是该机制受限于操作系统，而且很容易误报。所以很少被大家使用。
...
使用心跳之前，必须要明确数据格式，绝对不能混淆业务数据。一般在适配Plc等现成模块时，他们是有固定的数据格式，这时候你可以参阅[数据处理适配器](<..\四、数据处理适配器\4.2 Tcp适配器\7、模板解析“固定包头”数据适配器.md>)，快速的解析数据。
