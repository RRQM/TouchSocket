...
在 `TouchSocket` 包中默认集成了 `EFCore`，**如果不使用 `EFCore`，可安装纯净版 `TouchSocket.Pure` 代替 `TouchSocket`，这样可以忽略本章节直接按照 `SqlSugar` 官方文档使用**。
...
在 `TouchSocket` 框架中，已经推出 `SqlSugar` 拓展包 [TouchSocket.Extras.DatabaseAccessor.SqlSugar](https://www.nuget.org/packages/TouchSocket.Extras.DatabaseAccessor.SqlSugar)。
...
在 `TouchSocket` 框架中，推荐将拓展包 `TouchSocket.Extras.DatabaseAccessor.SqlSugar` 安装到 `TouchSocket.Core` 层中。
...
db =>
{
    db.Aop.OnLogExecuting = (sql, pars) =>
    {
        ...
        //方法二:TouchSocket对上述用法进行二次封装 建议方式 Sql参数会直接写入到生成Sql中
