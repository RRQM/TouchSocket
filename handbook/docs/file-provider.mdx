...
`TouchSocket` 框架提供了 `Func<FileProviderTypes, object, IFileProvider>` 委托供构造函数注入或解析服务，如：
...
`TouchSocket` 框架也提供了 `FS` 静态类方式创建，如：
...
在 TouchSocket `v3.2.0` 新增了 `IFormFile` 的 `ToByteArray` 拓展，如:
