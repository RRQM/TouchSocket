...
### 25.3.5 集成 `TouchSocket`

`Worker Service` 集成 `TouchSocket` 非常方便，只需要安装 `TouchSocket` 的包即可，并在 `Program.cs` 中调用 `.Inject()` 方法，如：
    ...
    public class Program
    {
        ...
        public static IHostBuilder CreateHostBuilder(string[] args) =>
            Host.CreateDefaultBuilder(args)
                ...
                .ConfigureServices((hostContext, services) =>
                {
                    // 以下代码可不用编写，TouchSocket 已实现自动注册 Worker;
                    // services.AddHostedService<Worker>();
                });
    }
}
...
集成 `TouchSocket` 后会自动扫描 `Worker` 类并实现自动注册。
...
`TouchSocket` 框架为 `BackgroundService` 提供了定时任务的支持。
...
- **第三步**：发布 `Worker Service`，可通过 `dotnet publish -c Release -o C:\TouchSocketWorker` 命令发布或通过 `Visual Studio 2019` 发布。
...
```cmd
sc.exe create TouchSocketWorkerServices binPath= C:\TouchSocketWorker\TouchSocketWorker.exe
```

注意`=`后面要有一个空格
创建成功后可通过 `sc.exe query TouchSocketWorkerServices` 查看服务状态。

- **第五步**

启动服务：`sc.exe start TouchSocketWorkerServices`，启动之后就可以在 `Windows` 服务工具中查看了。
