...
在 `TouchSocket` 框架中，默认集成了微软亲儿子：`EntityFramework Core` ，也就是通常数据库上下文指的是 `DbContext` 类或它的实现类。

## 9.1.2 `AppDbContext`

在我们实际项目开发过程中，使用 `EFCore` 提供的 `DbContext` 操作对象操作数据库有些繁琐和复杂，且默认不具备读写分离、多库等操作功能。

所以，`TouchSocket` 框架提供了 `AppDbContext<TDbContext, TDbContextLocator>` 数据库上下文，该上下文继承自 `DbContext`。
...
- `AppDbContext` 能够得到 `TouchSocket` 框架更多的功能支持

## 9.1.4 如何定义数据库上下文

在 `TouchSocket` 框架中了，提供了两种 `AppDbContext` 定义方式：
...
namespace TouchSocket.EntityFramework.Core
{
    ...
    public class TouchSocketDbContext : AppDbContext<TouchSocketDbContext>  // 继承 AppDbContext<> 类
    {
        /// <summary>
        /// 继承父类构造函数
        /// </summary>
        /// <param name="options"></param>
        public TouchSocketDbContext(DbContextOptions<TouchSocketDbContext> options) : base(options)
        {
        }
    }
}
...
:::important 特别注意
所有数据库上下文都应该在 `TouchSocket.EntityFramework.Core` 项目中创建。另外如果系统用到了多个数据库，那么从第二个开始必须指定数据库上下文定位器。关于 `TDbContextLocator` 将在下一章节 《[9.2 数据库上下文定位器](dbcontext-locator)》阐述。
:::

## 9.1.5 配置连接字符串

`TouchSocket` 框架提供多种数据库连接字符串配置方式：
...
```cs showLineNumbers  {12-19} title="TouchSocket.EntityFramework.Core\Startup.cs"
...
namespace TouchSocket.EntityFramework.Core
{
    ...
    public sealed class FurEntityFrameworkCoreStartup : AppStartup
    {
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                // 配置默认数据库
                options.AddDbPool<TouchSocketDbContext>(DbProvider.SqlServer, connectionMetadata:"连接字符串");
...
namespace TouchSocket.EntityFramework.Core
{
    ...
    public class TouchSocketDbContext : AppDbContext<TouchSocketDbContext>
    {
        /// <summary>
        /// 继承父类构造函数
        /// </summary>
...
- `Sqlite`：`Data Source=./TouchSocket.db`
- `MySql`：`Data Source=localhost;Database=TouchSocket;User ID=root;Password=000000;pooling=true;port=3306;sslmode=none;CharSet=utf8;`
- `SqlServer`：`Server=localhost;Database=TouchSocket;User=sa;Password=000000;MultipleActiveResultSets=True;`
...
在 `TouchSocket` 框架中，数据库上下文需定义在 `TouchSocket.EntityFramework.Core` 中，且每一个数据库上下文都必须拥有唯一的 `DbContextLocator` 定位器
...
```cs showLineNumbers  {11-13} title="TouchSocket\framework\TouchSocket.EntityFramework.Core\FurEntityFrameworkCoreStartup.cs"
using TouchSocket.DatabaseAccessor;
using Microsoft.Extensions.DependencyInjection;
...
```cs showLineNumbers 
options.AddDbPool<TouchSocketDbContext>(DbProvider.Sqlite); // 第一个数据库
...
在 `TouchSocket` 框架中，为了能够实现数据库的简单使用进行了注册封装，但有些时候，我们可能需要添加更多配置，这时就需要使用原生自定义配置方式，如：
...
`TouchSocket` 框架提供了快速解析连接字符串的静态方法，自动根据名称读取配置，自动解析 `[AppContext("...")]` 信息，如：
...
在 `TouchSocket` 框架中，数据库上下文是定义在 `TouchSocket.EntityFramework.Core` 项目层，并且该层不被 `TouchSocket.Application` 和 `TouchSocket.Core` 等层引用。

所以就不能直接在 `TouchSocket.Application` 项目层直接使用 `TouchSocket.EntityFramework.Core` 定义的数据库上下文。

`TouchSocket` 为了解决这个问题，提供了两种方式处理：
