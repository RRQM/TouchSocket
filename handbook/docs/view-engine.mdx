...
在 `TouchSocket` 框架中，底层集成了微软提供的 `Razor` 视图引擎组件并提供更加灵活方便的语法糖。
...
namespace TouchSocket.Application
{
    public class ViewEngineService : IDynamicApiController
    {
        ...
        public ViewEngineService(IViewEngine viewEngine)
        {
            ...
            var result = _viewEngine.RunCompile("Hello @Model.Name", new { Name = "TouchSocket" });
        }
    }
}
```
...
```cs showLineNumbers 
var result = "Hello @Model.Name".RunCompile(new { Name = "TouchSocket" });
```
...
```cs showLineNumbers 
var result = _viewEngine.RunCompileFromCached(@"<div>@System.IO.Path.Combine(""TouchSocket"", ""ViewEngine"")</div>", builderAction: builder =>
            {
...
```html showLineNumbers 
<div>TouchSocket\\ViewEngine</div>
```

另外，`TouchSocket` 提供多种方式加载程序集：
...
```cs showLineNumbers 
var result = _viewEngine.RunCompileFromCached(@"<div>@Path.Combine(""TouchSocket"", ""ViewEngine"")</div>", builderAction: builder =>
            {
...
`TouchSocket` 除了内置视图引擎之外，还支持以下几种模板替换，如：

```cs showLineNumbers 
// 提供数据模板方式
var str = "我叫{name}".Render(new Dictionary{ {"name", "TouchSocket"} });
var str = "我叫{Name}".Render(new { Name = "TouchSocket" });
var str = "我叫{Detail.Name}".Render(new { Detail = new { Name = "Furoin" } });

// 从配置读取方式
var str = "我叫#(TouchSocket:Address)".Render();
```
