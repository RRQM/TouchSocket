...
1. 必须完成第一次连接。
2. 必须是被动断开，如果是客户端主动调用`Close`、`Disposed`等方法主动断开的话，则不会生效。
3. 必须有显式的断开信息，也就是说，直接拔网线的话，**不会**立即生效，会等tcp保活到期后再生效。
...
绝对保活，是一种不依赖于任何事件的保活方式。只要连接链路在持续时间内没有数据交互，则视为断开，详情请看[健康活性检验插件](./tcpcommonplugins.mdx)


## 十、示例Demo

<CardLink link="https://gitee.com/RRQM_Home/TouchSocket/tree/master/examples/Tcp" isPro="true"/>
