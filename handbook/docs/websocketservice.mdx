...
一般的，在`IWebSocketHandshakedPlugin`插件中（即刚连接成功）调用`ReadAsync`方法来接收数据。
...
使用`ReadAsync`方式会阻塞`IWebSocketHandshakedPlugin`的插件传递链，在收到`WebSocket`消息时不会触发`IWebSocketReceivedPlugin`插件。
...
<CustomCodeBlock region="WebSocket服务器处理中继数据"/>
