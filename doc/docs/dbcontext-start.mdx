...
在本章节所在的类别中，所有数据库操作功能均需要依赖 `EntityFramework Core` 框架，但 `TouchSocket` 框架底层并未集成 `EntityFramework Core` 包，而是采用动态加载程序集方式自动载入。
...
这些数据库包应该安装在 `TouchSocket.EntityFramework.Core` 层。特殊情况需安装在 `TouchSocket.Core` 层中，如 `Mysql` `HasCharset()` 配置。
...
`TouchSocket` 框架提供多种方式配置数据库连接字符串：
...
[AppDbContext("Data Source=./TouchSocket.db", DbProvider.Sqlite)]
public class DefaultDbContext : AppDbContext<DefaultDbContext>
{
}
```
...
- `SqlServer`：`Server=localhost;Database=TouchSocket;User=sa;Password=000000;MultipleActiveResultSets=True;Encrypt=True;TrustServerCertificate=True;`
