...
/// <summary>
/// 提供 MqttRpc 协议消息的低分配序列化与反序列化能力。
/// </summary>
internal sealed class MqttRpcConverter
{
    ...
    private static ReadOnlySpan<byte> RtBytes => "rt"u8;
    ...
    /// <summary>
    /// 将请求序列化写入 <see cref="IBufferWriter{T}"/>。
    /// </summary>
    ...
    public void WriteRequest(IBufferWriter<byte> bufferWriter, string method, int? id, string responseTopic, object[] parameters, JsonSerializerOptions options)
    {
        ...
        writer.WriteString(RtBytes, responseTopic ?? string.Empty);
    ...
    /// <summary>
    /// 将响应序列化写入 <see cref="IBufferWriter{T}"/>。
    /// </summary>
    [UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "MqttRpc基础设施相信动态代码是有效的")]
    [UnconditionalSuppressMessage("AOT", "IL3050", Justification = "MqttRpc基础设施相信动态代码是有效的")]
    public void WriteResponse(IBufferWriter<byte> bufferWriter, int? id, object result, int errorCode, string errorMessage, JsonSerializerOptions options)
    {
        ...
        else
        {
            ...
            writer.WriteString(MessageBytes, errorMessage ?? string.Empty);
            writer.WriteEndObject();
        }
        writer.WriteEndObject();
    }
    ...
    /// 从字节数据中尝试读取请求。
    /// </summary>
    public bool TryReadRequest(ReadOnlySpan<byte> data, out InternalMqttRpcRequest request)
    {
        ...
        var result = new InternalMqttRpcRequest();
        var hasMethod = false;
        ...
        while (reader.Read())
        {
            ...
            else if (reader.ValueTextEquals(RtBytes))
            {
                reader.Read();
                result.Rt = reader.GetString();
            }
            else if (reader.ValueTextEquals(ParamsBytes))
            {
                reader.Read();
                result.Params = JsonElement.ParseValue(ref reader);
            }
            else
            {
                reader.Read();
    ...
    /// 从字节数据中尝试读取响应。
    /// </summary>
    public bool TryReadResponse(ReadOnlySpan<byte> data, out MqttRpcWaitResult response)
    {
        var reader = new Utf8JsonReader(data);
        if (!reader.Read() || reader.TokenType != JsonTokenType.StartObject)
        {
        ...
        var result = new MqttRpcWaitResult();
