...
`TouchSocket` 框架默认数据迁移的程序集为：`TouchSocket.Database.Migrations`，所以如果您改了程序集名称或通过 `Nuget` 方式安装的 `TouchSocket` 包，则需要配置迁移程序集名称：
...
Add-Migration v1.0.0 -Context TouchSocketDbContext
```
...
将 `程序包管理控制台` 默认项目设置为 `TouchSocket.Database.Migrations`
...
Microsoft.EntityFrameworkCore.Infrastructure[10403]
      Entity Framework Core 5.0.0-rc.1.20451.13 initialized 'TouchSocketDbContext' using provider 'Microsoft.EntityFrameworkCore.Sqlite' with options: SensitiveDataLoggingEnabled DetailedErrorsEnabled MaxPoolSize=100 MigrationsAssembly=TouchSocket.Database.Migrations
To undo this action, use Remove-Migration.
PM>
```

生成成功后，`TouchSocket.Database.Migrations` 项目下会新增 `Migrations` 文件夹（如果没有），同时本次的架构生成文件，如：
...
之后进入 `TouchSocket.Database.Migrations` 目录

```shell showLineNumbers 
cd TouchSocket.Database.Migrations
```

### 9.20.5.3 执行命令

```shell showLineNumbers 
dotnet ef migrations add v1.0.0 -s "../TouchSocket.Web.Entry"
```

```shell showLineNumbers 
dotnet ef database update -s "../TouchSocket.Web.Entry"
```

## 9.20.6 应用启动时自动生成数据库

`TouchSocket` 框架建议大家使用命令方式操作数据库，**完全不推荐自动化生成数据库**，但是有些特殊情况下，有这个必要，故将此功能写出：
...
public void Configure(IApplicationBuilder app, IHostingEnvironment env)
{
    ...
    if (env.IsDevelopment())
    {
        ...
        {
            var context = scope.ServiceProvider.GetRequiredService<TouchSocketDbContext>();
...
Microsoft.EntityFrameworkCore.Model.Validation[10400]
      Sensitive data logging is enabled. Log entries and exception messages may include sensitive application data; this mode should only be enabled during development.
Microsoft.EntityFrameworkCore.Infrastructure[10403]
      Entity Framework Core 6.0.8 initialized 'DefaultDbContext' using provider 'MySql.EntityFrameworkCore:6.0.4+MySQL8.0.30' with options: SensitiveDataLoggingEnabled DetailedErrorsEnabled MaxPoolSize=100 MigrationsAssembly=TouchSocket.TestMS.Database.Migrations 
...
相关 `Issue` 讨论：[https://gitee.com/rrqm_home/TouchSocket/issues/I5O5ER](https://gitee.com/rrqm_home/TouchSocket/issues/I5O5ER)
