...
`TouchSocket` 框架支持以上三种多租户实现方案，使用简单且容易维护。下面分别使用三种不同方式演示多租户方案用法。
...
```cs showLineNumbers  {6-7} title="TouchSocket.EntityFramework.Core\DbContexts\MultiTenantDbContext.cs"
...
namespace TouchSocket.EntityFramework.Core
{
    ...
    public sealed class FurEntityFrameworkCoreStartup : AppStartup
    {
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.AddDbPool<TouchSocketDbContext>();
...
```cs showLineNumbers  {8,12-28} title="TouchSocket.EntityFramework.Core\SeedDatas\TenantSeedData.cs"
using TouchSocket.DatabaseAccessor;
using Microsoft.EntityFrameworkCore;
using System;
...
{
    [AppDbContext("Sqlite3ConnectionString", DbProvider.Sqlite)]
    public class TouchSocketDbContext : AppDbContext<TouchSocketDbContext>, IMultiTenantOnTable, IModelBuilderFilter
    {
...
`base.Tenant` 只是 `TouchSocket` 框架提供的默认租户实现方法，如果不能满足业务需求，只需要在 `GetTenantId` 里面写你的业务代码即可，也就是无需调用 `base.Tenant`。如：
...
namespace TouchSocket.EntityFramework.Core
{
    ...
    public sealed class FurEntityFrameworkCoreStartup : AppStartup
    {
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddDatabaseAccessor(options =>
            {
                options.AddDb<TouchSocketDbContext>();
                options.AddDbPool<MultiTenantDbContext, MultiTenantDbContextLocator>();
            });
        }
    }
...
```cs showLineNumbers  {8,12-30} title="TouchSocket.EntityFramework.Core\SeedDatas\TenantSeedData.cs"
using TouchSocket.DatabaseAccessor;
using Microsoft.EntityFrameworkCore;
using System;
using System.Collections.Generic;
        ...
        public IEnumerable<Tenant> HasData(DbContext dbContext, Type dbContextLocator)
        {
            return new List<Tenant>
            {
                new Tenant
                {
                    ...
                    Host = "localhost:44313",
                    CreatedTime = DateTime.Parse("2020-10-06 20:19:07"),
                    ConnectionString = "Data Source=./TouchSocket.db" // 配置连接字符串
                },
                new Tenant
                {
                    TenantId = Guid.Parse("C5798CB6-16D6-0F42-EB56-59C695353BC0"),
...
{
    // 这里可不配置
    public class TouchSocketDbContext : AppDbContext<TouchSocketDbContext>, IMultiTenantOnDatabase
    {
        public TouchSocketDbContext(DbContextOptions<TouchSocketDbContext> options) : base(options)
        {
        }
...
`base.Tenant` 只是 `TouchSocket` 框架提供的默认租户实现方法，如果不能满足业务需求，只需要在 `GetDatabaseConnectionString` 里面写你的业务代码即可，也就是无需调用 `base.Tenant`。如：
...
{
    [AppDbContext("Sqlite3ConnectionString", DbProvider.Sqlite)]
    public class TouchSocketDbContext : AppDbContext<TouchSocketDbContext>, IMultiTenantOnSchema
    {
        public TouchSocketDbContext(DbContextOptions<TouchSocketDbContext> options) : base(options)
        {
        }
...
`base.Tenant` 只是 `TouchSocket` 框架提供的默认租户实现方法，如果不能满足业务需求，只需要在 `GetSchemaName` 里面写你的业务代码即可，也就是无需调用 `base.Tenant`。如：
...
默认情况下，`TouchSocket` 框架提供了内置的 `Tenant` 类型，方便大家快速实现 `SaaS` 多租户功能，如果需要自定义多租户 `Tenant` 类型，只需要启用以下配置即可：
...
`TouchSocket` 框架会在租户上下文第一次查询时候将租户表缓存起来，避免频发查询数据库，如果更新了租户表，则需要手动刷新租户信息，如：

```cs showLineNumbers 
using TouchSocket.DatabaseAccessor.Extensions;
