...
`TouchSocket` 框架提供四种数据库操作拦截器，可以通过拦截器动态修改数据库连接字符串，动态修改 sql，动态更改参数等操作。

`TouchSocket` 支持这四种拦截器：
...
namespace TouchSocket.DatabaseAccessor.Interceptors
{
...
namespace TouchSocket.DatabaseAccessor
{
...
`TouchSocket` 框架中为所有 `AppDbContext` 子类都提供了三个可重写的方法，这三个方法分别由三个事件触发：
...
services.AddDbPool<TouchSocketDbContext>(interceptors: new IInterceptor[] {
