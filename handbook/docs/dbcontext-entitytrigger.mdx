...
在最新的 `TouchSocket` 的 `1.1.6+` 版本中，新增了 `IEntityChangedListener` 实体数据监听接口，可以监听 `EFCore` 任何实体表 `增删改` 操作。
...
在 `TouchSocket` 框架中，默认不启用实体数据监听器，如想启用，只需要在 `数据库上下文` 构造函数中启用即可：
...
默认情况下，`TouchSocket` 框架会对所有新增、更新、编辑的实体进行监听，有些时候我们无需监听特定实体，只需要在实体上贴 `[SuppressChangedListener]` 特性即可。
