...
`TouchSocket` 框架每一个数据库操作都支持异步方式，由于篇幅有限，就不列举异步方式了。
...
`TouchSocket` 框架提供非常灵活的在 `Linq` 中使用标量函数的方法。如果像使用这样的方式，需要满足以下两个条件：
...
var result = query.Where(u => u.Name.Equals("TouchSocket")).ToList();
```
