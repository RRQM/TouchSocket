...
namespace RRQMCore.Converter
{
    ...
    /// String类型数据转换器
    /// </summary>
    public class StringConverter : RRQMConverter<string>
    {
        /// <summary>
        /// 构造函数
        /// </summary>
    ...
    public class StringToPrimitiveConverter : IConverter<string>
    {
        ...
        {
            if (targetType.IsPrimitive || targetType == RRQMCoreUtility.stringType)
            {
                return RRQMCoreExtensions.TryParseToType(source, targetType, out target);
            }
            target = default;
            return false;
        }
        ...
        public bool TryConvertTo(object target, out string source)
        {
            if (target != null)
            {
                ...
                if (type.IsPrimitive || type == RRQMCoreUtility.stringType)
                {
