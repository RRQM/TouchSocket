...
在 `TouchSocket` 框架中，提供了更懒的方式写入日志，也就是通过字符串拓展的方式写入，如：
...
info: TouchSocket.EventBus.EventBusHostedService[0]
...
info: Microsoft.Hosting.Lifetime[0]
      Content root path: C:\Workplaces\TouchSocket\samples\TouchSocket.Web.Entry\
```
...
以下内容仅限 `TouchSocket 4.5.0 +` 版本使用。

:::

在 `ASP.NET Core` 默认控制台日志相对简洁，并未包含常见的日志时间、线程 `Id` 等，而且自定义模板也相对复杂，所以 `TouchSocket 4.5.0+` 版本提供了简化配置，如：
...
// 输出为 JSON 格式，TouchSocket 4.5.2+
services.AddConsoleFormatter(options =>
{
    options.MessageFormat = LoggerFormatter.Json;
    // TouchSocket 4.8.0+ 新增 JSON 美化输出
...
以下内容仅限 `TouchSocket 4.8.1.3 +` 版本使用。
...
默认情况下，`TouchSocket` 提供了标准的日志输出模板，如：

```bash showLineNumbers
2022-07-23T20:16:29.3459053+08:00	[INF]	[TouchSocket.EventBus.EventBusHostedService]	[0]	EventBus Hosted Service is running.
...
2022-07-23T20:16:29.5829377+08:00	[INF]	[Microsoft.Hosting.Lifetime]	[0]	Content root path: C:\Workplaces\TouchSocket\samples\TouchSocket.Web.Entry\
```
...
将日志输出到数据库中也是非常常见的需求，`TouchSocket` 把该功能做到了非常简单，支持任何存储介质。
...
`TouchSocket` 也提供了运行时动态创建日志提供器并写入：
...
`TouchSocket` 也提供了原生 `services.AddLogging(builder => {})` 方式配置，如
...
在 `TouchSocket 3.9.1` 版本新增了 `[LoggingMonitor]` 特性，支持在控制器或操作中贴该特性，可以实现强大的请求日志监听，方便测试，如：
...
- `[LoggingMonitor]` 支持以下配置：
  ...
  - `WithReturnValue`：是否包含返回值打印，`bool` 类型，默认 `true`，`TouchSocket 4.3.9+ 有效`
  - `ReturnValueThreshold`：配置返回值字符串阈值，`int` 类型，默认 `0` 全量输出，`TouchSocket 4.3.9+ 有效`
  - `JsonBehavior`：配置 `LoggingMonitor` `Json` 输出行为，默认 `None`，`TouchSocket 4.5.2+` 有效
  - `JsonIndented`：配置 `LoggingMonitor` `Json` 格式化行为，`bool` 类型，默认 `false`，`TouchSocket 4.8.0+` 有效
...
:::tip `TouchSocket 4.0.2` 新推荐配置

在 `TouchSocket 4.0.2` 版本中新增了非常灵活方便的 `services.AddMonitorLogging()` 服务配置，可在配置中随意控制哪个类哪个方法启用或不启用。
...
```json showLineNumbers {2,3,4-6}
{
  "Logging": {
    "Monitor": {
      ...
      "WithReturnValue": true, // 配置是否包含返回值，默认 `true`，TouchSocket 4.3.9+ 有效
      "ReturnValueThreshold": 0, // 配置返回值字符串阈值，默认 0，全量输出，TouchSocket 4.3.9+ 有效
      "JsonBehavior": "None", // 配置 LoggingMonitor Json 输出行为，默认 None，TouchSocket 4.5.2+ 有效
      "JsonIndented": false, // 配置 LoggingMonitor Json 格式化行为，默认 false，TouchSocket 4.8.2+ 有效
      "MethodsSettings": [
        // 配置被监视方法更多信息，TouchSocket 4.3.9+ 有效
        {
          "FullName": "TouchSocket.Application.TestLoggerServices.MethodName", // 方法完全限定名
          "WithReturnValue": true, // 配置是否包含返回值，默认 `true`，TouchSocket 4.3.9+ 有效
          "ReturnValueThreshold": 0, // 配置返回值字符串阈值，默认 0，全量输出，TouchSocket 4.3.9+ 有效
          "JsonIndented": false // 配置 LoggingMonitor Json 格式化行为，默认 false，TouchSocket 4.8.2+ 有效
        }
...
`IncludeOfMethods` 和 `ExcludeOfMethods` 方法签名格式为：`类完全限定名.方法名`，如：`TouchSocket.Application.TestNamedServices.GetName`，`TouchSocket.Application.TestNamedServices` 是类名，`GetName` 是方法名。
...
services.AddMonitorLogging(options =>
{
    ...
    options.JsonIndented = true;    // 是否美化 JSON，TouchSocket 4.8.0+ 版本有效
});

// 局部
[LoggingMonitor(JsonBehavior = TouchSocket.Logging.JsonBehavior.OnlyJson)]
// 是否美化 JSON，TouchSocket 4.8.0+ 版本有效
[LoggingMonitor(JsonBehavior = TouchSocket.Logging.JsonBehavior.OnlyJson, JsonIndented = true)]
```
...
以下内容仅限 `TouchSocket 4.5.9 +` 版本使用。

:::

在 `TouchSocket 4.5.9+` 版本新增了 `WriteFilter` 过滤功能，可根据自定义逻辑自定义过滤拦截：
...
在 `TouchSocket` 框架中默认集成了 `MiniProfiler` 组件并与 `Swagger` 进行了结合，如需打印日志或调试代码，只需调用以下方法即可：
...
在 `TouchSocket v3.5.3+` 新增了 `TP.Wrapper(...)` 规范模板，使用如下：
...
`TouchSocket 3.9.1` 之前版本使用 `[属性名]` 开头。
...
services.AddFileLogging("infomation.log", options =>
{
    options.WriteFilter = (logMsg) =>
    {
        ...
        return logMsg.Context.Get("Name") == "TouchSocket";
    };
});
