...
`BytePool`在默认情况提供了一个`BytePool.Default`的默认静态实例。这是整个进程可以共享使用的。
...
内存块就是可以使用的字节数组。框架提供了`ByteBlock`和`ValueByteBlock`两种内存块。后者是值类型的Handle，所以在创建时不能使用`new ValueByteBlock()`。而是要指定长度。
