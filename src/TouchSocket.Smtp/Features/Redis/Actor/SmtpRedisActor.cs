...
namespace TouchSocket.Smtp.Redis
{
    ...
    /// SmtpRedisActor
    /// </summary>
    public class SmtpRedisActor : ISmtpRedisActor
    {
        /// <summary>
        /// SmtpRedisActor
        /// </summary>
        /// <param name="smtpActor"></param>
        public SmtpRedisActor(ISmtpActor smtpActor)
        {
        ...
        public void ClearCache()
        {
            ...
            var waitData = this.SmtpActor.WaitHandlePool.GetWaitData(package);
            try
            {
                using (var byteBlock = new ByteBlock())
                {
                    package.Package(byteBlock);
                    this.SmtpActor.Send(this.m_redis_Request, byteBlock);
                }
                switch (waitData.Wait(this.Timeout))
                {
                    ...
                    case WaitDataStatus.Overtime: throw new TimeoutException(TouchSocketSmtpStatus.Overtime.GetDescription());
        ...
        public ICacheEntry<string, byte[]> GetCache(string key)
        {
            ...
            try
            {
                ...
                switch (waitData.Wait(this.Timeout))
                {
                    ...
                    case WaitDataStatus.Disposed:
                    default:
                        throw new TimeoutException(TouchSocketSmtpStatus.UnknownError.GetDescription());
                }
            }
            finally
            {
        ...
        public bool InputReceivedData(SmtpMessage message)
        {
            if (message.ProtocolFlags == this.m_redis_Request)
            {
                ...
                using (var byteBlock = new ByteBlock())
                {
                    ...
                    this.SmtpActor.Send(this.m_redis_Response, byteBlock);
                }
                return true;
            }
            else if (message.ProtocolFlags == this.m_redis_Response)
            {
                ...
                this.SmtpActor.WaitHandlePool.SetRun(waitResult);
                return true;
            }
            return false;
        }
        ...
        public bool RemoveCache(string key)
        {
            ...
            try
            {
                ...
                switch (waitData.Wait(this.Timeout))
                {
                    ...
                    case WaitDataStatus.Overtime: throw new TimeoutException(Resources.TouchSocketSmtpStatus.Overtime.GetDescription());
                    case WaitDataStatus.Canceled: return false;
                    case WaitDataStatus.Default:
                    case WaitDataStatus.Disposed:
                    default:
                        throw new TimeoutException(Resources.TouchSocketSmtpStatus.UnknownError.GetDescription());
                }
            }
            finally
            {
