...
client.Connecting = (client, e) => { };//即将连接到服务器，此时已经创建管道，但是还未建立连接
client.Connected = (client, e) => { };//成功连接到服务器
client.Disconnecting = (client, e) => { };//即将从服务器断开连接。此处仅主动断开才有效。
...
client.Connecting = (client, e) => { };//即将连接到服务器，此时已经创建socket，但是还未建立tcp
client.Connected = (client, e) => { };//成功连接到服务器
client.Disconnecting = (client, e) => { };//即将从服务器断开连接。此处仅主动断开才有效。
client.Disconnected = (client, e) => { };//从服务器断开连接，当连接不成功时不会触发。
