...
- `AES` 加解密
- `JWT` 加解密
- `PBKDF2` 加密（`TouchSocket v2.12 +` 版本已移除）
- `RSA` 加解密
...
// 输出 16位 MD5 加密，TouchSocket 4.2.6+ 版本
...
```cs showLineNumbers
// 测试 DESC 加解密
var descHash = DESCEncryption.Encrypt("百小僧", "TouchSocket"); // 加密
var str = DESCEncryption.Decrypt(descHash, "TouchSocket");  // 解密
return (descHash, str);
```
...
`JWTEncryption` 加解密并未包含在 `TouchSocket` 框架中，需要安装 `TouchSocket` 框架提供的 `TouchSocket.Extras.Authentication.JwtBearer` 拓展包。

:::

### 20.3.5 `PBKDF2` 加密

**`TouchSocket v2.12 +` 版本已移除。**
...
`TouchSocket` 框架底层不内置 `RSA` 签名和校验功能，如需添加该功能可查阅开发者提交的代码：[查看 RSA 签名和校验](https://gitee.com/rrqm_home/TouchSocket/pulls/349)

:::

## 20.4 字符串拓展方式

`TouchSocket` 框架也提供了字符串拓展方式进行 `MD5加密、AES/DESC加解密、RSA加解密`。

```cs showLineNumbers
using TouchSocket.DataEncryption.Extensions;

// MD5 加密
var s = "TouchSocket".ToMD5Encrypt();
var b = "TouchSocket".ToMD5Compare(s);   // 比较

// AES加解密
var s = "TouchSocket".ToAESEncrypt("sfdsfdsfdsfdsfdsfdsfdsfdsfdfdsfdsfdfdfdfd");
var str = s.ToAESDecrypt("sfdsfdsfdsfdsfdsfdsfdsfdsfdfdsfdsfdfdfdfd");

// DESC 加解密
var s = "TouchSocket".ToDESCEncrypt("sfdsfdsfdsfdsfdsfdsfdsfdsfdfdsfdsfdfdfdfd");
var str = s.ToDESCDecrypt("sfdsfdsfdsfdsfdsfdsfdsfdsfdfdsfdsfdfdfdfd");

// PBKDF2 加密（`TouchSocket v2.12 +` 版本已移除！！！！！！！！）
var s = "TouchSocket".ToPBKDF2Encrypt();
var b = "TouchSocket".ToPBKDF2Compare(s);   // 比较

// RSA 加解密
var (publicKey, privateKey) = RSAEncryption.GenerateSecretKey(2048);  //生成 RSA 秘钥 秘钥大小必须为 2048 到 16384，并且是 8 的倍数
var s= "TouchSocket".ToRSAEncrpyt(publicKey);  // 加密
var str=s.ToRSADecrypt(privateKey);  // 解密
```

## 20.5 `SM2`、`SM3`，`SM4` 国密

`TouchSocket` 框架未内置国密算法 `SM2-4`，但是已有开发者贡献实现并开源，可查阅 [Gitee 仓库](https://gitee.com/xxcxy/SM/tree/master/GfsDemo/Smcrypto)，感谢 `QQ（373696184）形影相印²º²²` 贡献
