...
以下内容仅限 `TouchSocket 3.6.3 +` 版本使用。

:::

:::tip 推荐使用脚手架

`TouchSocket` 官方提供了非常灵活方便的脚手架，可以快速的创建多层架构项目。
...
// 创建控制台项目
dotnet new console -n HelloTouchSocket
```

:::

## 2.0.3 添加 `TouchSocket` 依赖包
...
传入 `RunOptions` 对象相当于自由定义和控制，也就是除了默认集成了 `TouchSocket` 以外，没有注册任何功能。

- 仅集成 `TouchSocket` 的默认配置
...
所以 `TouchSocket` 推荐下面更加灵活且易维护的方式，`Program.cs` 只需一句话即可：

:::tip 推荐使用组件启动

`TouchSocket 3.7.3+` 官方提供了非常灵活方便的组件化启动配置服务。
...
## 2.0.8 支持 `TouchSocket` 所有功能

`Serve.Run()` 看似非常简单，实则非常灵活，而且支持 `TouchSocket` 和 `.NET` 所有功能。
...
public class HelloService
{
    public string Say()
    {
        return "Hello, TouchSocket " + App.Configuration["Logging:LogLevel:Default"];
    }
}
```
...
在 `TouchSocket` 中可以派生自 `AppStartup` 可以实现更多配置，如：
...
是不是非常强大啊，`Serve.Run()` 虽然简单，但是 100% 支持 `TouchSocket` 和 `.NET` 所有功能。尽情去体验吧！
