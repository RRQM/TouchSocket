...
在 `TouchSocket` 系统中，`脱敏处理` 指的是对不符合系统合法词汇检测验证。

## 30.2 如何使用

`TouchSocket` 框架内置了一套默认的脱敏词汇脱敏处理机制，并且提供自定义操作。
...
在 `TouchSocket 3.8.9+` 版本支持 `|` 分隔符进行分割，也同时支持 `换行` 和 `|` 混用，如：
...
`TouchSocket` 框架提供了 `[SensitiveDetection]` 验证特性，可以对参数、属性进行脱敏验证，用法和 `[DataValidation]` 一致，如：
...
`TouchSocket` 框架也提供了 `ISensitiveDetectionProvider` 服务进行手动脱敏验证处理，如：
...
`TouchSocket` 框架也提供了替换脱敏词汇的特性支持，如：
...
在 `TouchSocket 3.8.8+` 版本后支持方法特性直接替换
...
`TouchSocket` 框架除了内置了一套默认的 `脱敏处理` 程序，也支持自定义脱敏处理程序。
