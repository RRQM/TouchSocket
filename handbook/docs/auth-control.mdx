...
- 安装 `TouchSocket.Extras.Authentication.JwtBearer` 拓展包
...
using TouchSocket.Authorization;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Http;
using System.Threading.Tasks;

namespace TouchSocketApi.Web.Core;
...
`TouchSocket` 框架为了方便开发，已经自动添加了 `Jwt` 默认配置。建议每个项目都应该单独配置 `IssuerSigningKey`，`ValidIssuer`，`ValidAudience` 这三个。否则同样用了 `TouchSocket` 框架生成的 `Token` 可能存在相互访问各自系统的风险。
...
`TouchSocket` 框架提供了非常灵活的高级策略鉴权和授权方式，通过该策略授权方式可以实现任何自定义授权。

### 15.3.1 `AppAuthorizeHandler`

`TouchSocket` 框架提供了 `AppAuthorizeHandler` 策略授权处理程序提供基类，只需要创建自己的 `Handler` 继承它即可。如：`JwtHandler`：
...
using TouchSocket.Authorization;
using TouchSocket.Core;
using TouchSocket.DataEncryption;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Http;
...
`TouchSocket` 框架提供了 `vue/react/angular` 客户端请求参考代码：**[https://gitee.com/rrqm_home/TouchSocket/tree/v4/clients](https://gitee.com/rrqm_home/TouchSocket/tree/v4/clients)**
...
如果 `Token` 过期，那么 `TouchSocket` 将自动根据有效期内的 `refreshToken` 自动生成新的 `AccessToken`，并在 **响应报文头** 中返回，如：
