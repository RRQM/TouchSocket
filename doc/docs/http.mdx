...
- **新特性**
  ...
  - &nbsp;<Tag>新增</Tag> 远程请求 `[QueryString]` 特性添加时间格式化 `Format` 属性 <sup>4.8.1.2</sup> <sup>⏱️2022.11.24</sup> [!670](https://gitee.com/rrqm_home/TouchSocket/pulls/670)
...
以下内容仅限 `TouchSocket 1.16.0 +` 版本使用。
...
{
    [Get("https://www.TouchSocket.icu/get")]
    Task<Result> GetXXXAsync();

    [Post("https://www.TouchSocket.icu/post")]
    Task<Result> PostXXXAsync();

    [Put("https://www.TouchSocket.icu/put")]
    Task<Result> PutXXXAsync();

    [Delete("https://www.TouchSocket.icu/delete")]
    Task<Result> DeleteXXXAsync();

    [Patch("https://www.TouchSocket.icu/patch")]
    Task<Result> PatchXXXAsync();

    [Head("https://www.TouchSocket.icu/head")]
...
using TouchSocket.DynamicApiController;
using TouchSocket.RemoteRequest.Extensions;
...
```cs showLineNumbers
var response = await "https://www.TouchSocket.icu/get".GetAsync();

var response = await "https://www.TouchSocket.icu/post".PostAsync();

var response = await "https://www.TouchSocket.icu/put".PutAsync();

var response = await "https://www.TouchSocket.icu/delete".DeleteAsync();

var response = await "https://www.TouchSocket.icu/patch".PatchAsync();

var response = await "https://www.TouchSocket.icu/head".HeadAsync();
```

需引入 `using TouchSocket.RemoteRequest.Extensions` 命名空间。
...
// 手动指定发送特定请求
var response = await "https://www.TouchSocket.icu/post".SetHttpMethod(HttpMethod.Post)
                                                  .SendAsync();
```
...
```cs showLineNumbers
// 该方式在 TouchSocket v3.0.0 已移除，多此一举了
await "".SetRequestUrl("https://www.TouchSocket.icu/");
```

### 19.4.3 设置请求方式

```cs showLineNumbers
await "https://www.TouchSocket.icu/post".SetHttpMethod(HttpMethod.Get);
```
...
```cs showLineNumbers
// 字典方式
await "https://www.TouchSocket.icu/post/{id}?name={name}&id={p.Id}".SetTemplates(new Dictionary<string , object> {
    { "id", 1 },
    { "name", "TouchSocket" },
...
// 对象/匿名对象方式
await "https://www.TouchSocket.icu/post/{id}?name={name}".SetTemplates(new {
    id = 1,
    name = "TouchSocket"
});
```
...
```cs showLineNumbers
// 字典方式
await "https://www.TouchSocket.icu/post".SetHeaders(new Dictionary<string , object> {
    { "Authorization", "Bearer 你的token"},
    { "X-Authorization", "Bearer 你的刷新token"}
});

// 对象/匿名对象方式
await "https://www.TouchSocket.icu/post".SetHeaders(new {
    Authorization = "Bearer 你的token"
});
```
...
```cs showLineNumbers
// 字典方式
await "https://www.TouchSocket.icu/get".SetQueries(new Dictionary<string , object> {
    { "id", 1 },
    { "name", "TouchSocket"}
});

// 对象/匿名对象方式
await "https://www.TouchSocket.icu/get".SetQueries(new {
    id = 1,
    name = "TouchSocket"
});

// TouchSocket 4.7.3+ 新增忽略 null 值重载
await "https://www.TouchSocket.icu/get".SetQueries(new {
    id = 1,
    name = "TouchSocket",
...
最终输出格式为：`https://www.TouchSocket.icu/get?id=1&name=TouchSocket`。
...
以下内容仅限 `TouchSocket 4.3.8 +` 版本使用。

:::

```cs showLineNumbers
await "https://www.TouchSocket.icu".SetClient(() => new HttpClient());
```
...
```cs showLineNumbers
// 传入对象
await "https://www.TouchSocket.icu/api/user/add".SetBody(new User { Id = 1, Name = "TouchSocket" });

// 配置 Content-Type
await "https://www.TouchSocket.icu/api/user/add".SetBody(new { Id = 1, Name = "TouchSocket" }, "application/json");

// 设置 Encoding 编码
await  "https://www.TouchSocket.icu/api/user/add".SetBody(new User { Id = 1, Name = "TouchSocket" }, "application/json", Encoding.UTF8);

// 处理 application/x-www-form-urlencoded 请求
await "https://www.TouchSocket.icu/api/user/add".SetBody(new Dictionary<string , object> {
    { "Id", 1 },
    { "Name", "TouchSocket"}
}, "application/x-www-form-urlencoded");

// 处理 application/xml、text/xml
await "https://www.TouchSocket.icu/api/user/add".SetBody("<SomeDto><SomeTag>somevalue</SomeTag></SomeDto>", "application/xml");
```
...
```cs showLineNumbers
await "https://www.TouchSocket.icu/post".SetContentType("application/json");
```

### 19.4.10 设置内容编码

```cs showLineNumbers
await "https://www.TouchSocket.icu/post".SetContentEncoding(Encoding.UTF8);
```

### 19.4.11 设置 `JSON` 序列化提供程序

`TouchSocket` 默认情况下采用 `System.Text.Json` 进行 `JSON` 序列化处理，如需设置第三方 `JSON` 提供器，则可以通过以下配置：

```cs showLineNumbers
// 泛型方式
await "https://www.TouchSocket.icu/api/user/add".SetJsonSerialization<NewtonsoftJsonSerializerProvider>();

// 非泛型方式
await "https://www.TouchSocket.icu/api/user/add".SetJsonSerialization(typeof(NewtonsoftJsonSerializerProvider));

// 添加更多配置
await "https://www.TouchSocket.icu/api/user/add".SetJsonSerialization<NewtonsoftJsonSerializerProvider>(new JsonSerializerSettings {

});

// 比如配置缺省的序列化选项
await "https://www.TouchSocket.icu".SetJsonSerialization(default, new JsonSerializerOptions
    {
        // 中文乱码
...
```cs showLineNumbers
await "https://www.TouchSocket.icu/api/user/add".SetValidationState();

// 设置不验证 null 值
await "https://www.TouchSocket.icu/api/user/add".SetValidationState(includeNull: true);
```
...
```cs showLineNumbers
await "https://www.TouchSocket.icu/".OnRequesting((client, req) => {
...
```cs showLineNumbers
await "https://www.TouchSocket.icu/".OnClientCreating(client => {
...
```cs showLineNumbers
await "https://www.TouchSocket.icu/".OnResponsing((client, res) => {
...
```cs showLineNumbers
await "https://www.TouchSocket.icu/".OnException((client, res, errors) => {
    // res 为 HttpResponseMessage 对象
});
```
...
```cs showLineNumbers
// HttpResponseMessage
var res = await "https://www.TouchSocket.icu/".GetAsync();

// Stream
var (stream, encoding) = await "https://www.TouchSocket.icu/".GetAsStreamAsync();

// T
var user = await "https://www.TouchSocket.icu/".GetAsAsync<User>();
...
:::warning `TouchSocket 4.4.0` 以下版本

在 `TouchSocket 4.4.0+` 版本移除了 `.SetBodyBytes` 方式，原因是拓展性太差，**新版本请使用 `.SetFiles` 方式**。
...
// 支持单文件，TouchSocket 4.5.8 版本支持 Stream 方式更新
var res = await "https://www.TouchSocket.icu/upload".SetContentType("multipart/form-data")
                                               .SetBodyBytes(("键", fileStream, "文件名")).PostAsync();

// 支持多个文件，TouchSocket 4.5.8 版本支持 Stream 方式更新
var res = await "https://www.TouchSocket.icu/upload".SetContentType("multipart/form-data")
...
:::tip `TouchSocket 4.4.0+` 版本

如果使用 `TouchSocket 4.4.0+` 版本，请使用以下的 `.SetFiles` 替代 `.SetBodyBytes` 操作。
...
```cs showLineNumbers
var res = await "https://www.TouchSocket.icu/upload".SetRequestScoped(services);
```
...
```cs showLineNumbers
var res = await "#(TouchSocket:Address)/upload".GetAsync();
```

```json showLineNumbers
{
  "TouchSocket": {
    "Address": "https://www.TouchSocket.icu"
  }
}
```
...
在 `TouchSocket v2.18+` 版本支持配置重试策略，如：

```cs showLineNumbers
var res = await "https://www.TouchSocket.icu".SetRetryPolicy(3, 1000).GetAsync();
```
...
在 `TouchSocket v3.2.0+` 版本支持`GZip` 压缩，如：

```cs showLineNumbers
var res = await "https://www.TouchSocket.icu".WithGZip().GetAsync();
```

### 19.4.24 设置 `Url` 转码

过去版本会对所有的 `Url` 进行 `Uri.EscapeDataString` 转码，在 `TouchSocket v3.8.0+` 版本支持 `Url` 转码设置，如：

```cs showLineNumbers
var res = await "https://www.TouchSocket.icu".WithEncodeUrl(false).GetAsync();
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://www.TouchSocket.icu/get/{id}?name={name}&number={p.PersonDetail.PhonNumber}")]
...
`TouchSocket` 框架远程请求代理模式提供三种方式设置请求报文头：
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://www.TouchSocket.icu/get/{id}?name={name}"), Headers("key2","value2")]
    Task<HttpResponseMessage> GetXXXAsync(int id, string name);

    [Get("https://www.TouchSocket.icu")]
...
public interface IHttp : IHttpDispatchProxy
{
    [Get("https://www.TouchSocket.icu/get/{id}?name={name}")]
    Task<HttpResponseMessage> GetXXXAsync(int id, string name);

    [Get("https://www.TouchSocket.icu/get/{p.Id}?name={p.Name}")]
    ...
    // TouchSocket 4.8.1.4 新增 [QueryString(Format)] 配置时间类型格式化
    [Get("https://www.TouchSocket.icu/get")]
    ...
    // TouchSocket 4.7.3 新增 IgnoreNullValueQueries 配置忽略空值
    [Get("https://www.TouchSocket.icu/get", IgnoreNullValueQueries = true)]
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://www.TouchSocket.icu/post"), JsonSerialization(typeof(NewtonsoftJsonSerializerProvider))]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
...
在 `TouchSocket 4.4.0+` 版本移除了 `[BodyBytes]` 方式，原因是拓展性太差，**新版本请使用 `HttpFile` 方式**。
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://www.TouchSocket.icu/upload", ContentType = "multipart/form-data")] // bytes 可以通过 File.ReadAllBytes(文件路径) 获取
...
如果使用 `TouchSocket 4.4.0+` 版本，请使用以下的 `HttpFile` 替代 `[BodyBytes]` 操作。请求有额外参数时 `HttpFile` 必须设置 `fileName` 值。
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("#(TouchSocket:Address)/upload")]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://www.TouchSocket.icu"), RetryPolicy(3, 1000)]    // 支持局部
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
在 `TouchSocket v3.2.0+` 版本支持 `GZip`，如：

```cs showLineNumbers
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://www.TouchSocket.icu", WithGZip = true)]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
public interface IHttp : IHttpDispatchProxy
{
    [Post("https://www.TouchSocket.icu", WithEncodeUrl = false)]
    Task<HttpResponseMessage> PostXXXAsync([Body]User user);
}
```
...
`TouchSocket` 框架也提供了多个请求客户端配置，可以为多个客户端请求配置默认请求信息，目前支持四种模式进行配置。
...
services.AddRemoteRequest(options=>
{
    // 默认 HttpClient 在 TouchSocket 框架内部已经配置了该操作
...
以下内容仅限 `TouchSocket v3.6.6+` 版本使用。
...
`TouchSocket` 提供了非常方便的请求并且序列化请求结果 `PostAsAsync<T>`
...
`TouchSocket` 框架内部默认不提供同步请求操作，建议总是使用异步的方式请求。如在不能使用异步的情况下，可自行转换为同步执行。如：
...
在 `TouchSocket` 框架底层中，`HttpClient` 对象默认通过 `IHttpClientFactory` 创建的，只要发送请求就会自动打印日志，如：
...
var response = await "https://www.TouchSocket.icu/".GetAsync();
