...
namespace XUnitTest.Core
{
    public class TestMethod
    {
        ...
        public void Method1ShouleBeOk()
        {
            ...
            Assert.True(method.TaskType == TaskType.None);
        ...
        [Fact]
        public async void Method3ShouleBeOk()
        {
            Method method = GetMethod("Method3");
            Assert.True(!method.HasReturn);
            Assert.True(!method.IsByRef);
            Assert.True(method.ReturnType == null);
            Assert.True(method.TaskType == TaskType.Task);
        ...
        public async void Method4ShouleBeOk()
        {
            ...
            Assert.True(method.TaskType == TaskType.TaskObject);

            var objs = new object[] { 10 };
            Assert.True((int)method.Invoke(new MyMethod(), objs) == 11);
            Assert.True((int)objs[0] == 11);

            Assert.True((int)await method.InvokeObjectAsync(new MyMethod(), objs) == 12);
            Assert.True((int)objs[0] == 12);
        }
        [Fact]
        public async void Method5ShouleBeOk()
        {
