...
<CardLink link="examples/Rpc/GenerateProxyFromServerConsoleApp"/>
...
<CustomCodeBlock region="代理生成基本示例"/>
...
<CustomCodeBlock region="代理生成使用CodeGenerator静态类"/>
...
<CustomCodeBlock region="代理生成直接添加代理类型"/>

或者直接按程序集添加

<CustomCodeBlock region="代理生成按程序集添加"/>

#### 2.2.2 通过特性标记添加

在需要代理的类上面声明RpcProxy标签，然后也可以重新指定代理类名。

<CustomCodeBlock region="代理生成通过特性标记添加"/>
...
<CustomCodeBlock region="代理生成类型排除"/>

或者直接按程序集排除

<CustomCodeBlock region="代理生成按程序集排除"/>
...
<CustomCodeBlock region="代理生成自定义RpcAttribute示例"/>
