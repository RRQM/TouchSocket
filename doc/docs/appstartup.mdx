...
```cs showLineNumbers  {18} title="TouchSocket.Web.Entry\Program.cs"
...
```cs showLineNumbers  {9,13} title="TouchSocket.Web.Entry\Startup.cs"
...
在 `TouchSocket` 框架中，提供了更为灵活的 `Startup` 类配置方式，**无需在 `Web 启用层` 中配置，可将配置放到任何项目层。**
...
所以，`TouchSocket` 提供了更加灵活的配置方式：`AppStartup`。
...
在 `TouchSocket v3.6.3+` 版本之后无需创建空 `Startup.cs` 类，内部已实现 `FakeStartup` 模式。

:::

建议 `Web` 启动层的 `Startup.cs` 保持为空方法体，如：

```cs showLineNumbers {9-11,13-15} title="TouchSocket.Web.Entry\Startup.cs"
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.Extensions.DependencyInjection;
...
将所有 `Web 应用层` 配置迁移到 `TouchSocket.Web.Core.Startup.cs` 中，如：

```cs showLineNumbers  title="TouchSocket.Web.Core\Startup.cs"
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Hosting;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
...
所以，`TouchSocket` 框架提供了更加灵活的方式配置 `appsettings.json`，**只需要在任何项目层根目录下创建 `.json` 文件即可。`TouchSocket` 框架最后会自动合并所有分散的配置文件。**

如我们在 `TouchSocket.EntityFramework.Core` 层创建 `dbsettings.json` 配置数据库连接字符串，如：

```json showLineNumbers  title="TouchSocket.EntityFramework.Core\dbsettings.json"
{
  "ConnectionStrings": {
