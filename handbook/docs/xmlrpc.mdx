...
.ConfigureRpcStore(store =>
{
    store.RegisterServer<XmlServer>();//注册服务

#if DEBUG
    //下列代码，会生成客户端的调用代码。
    var codeString = store.GetProxyCodes("XmlServerProxy", typeof(XmlRpcAttribute));
    File.WriteAllText("../../../XmlServerProxy.cs", codeString);
#endif
});
```
