...
<CustomCodeBlock region="依赖属性常规属性示例"/>
...
<CustomCodeBlock region="依赖属性内部声明类定义"/>
...
<CustomCodeBlock region="依赖属性扩展声明类定义"/>

那么这时候，**MyDependencyObject**对象即可赋值和获取**MyProperty2**的属性。

<CustomCodeBlock region="依赖属性扩展声明示例"/>
...
<CustomCodeBlock region="依赖属性应用场景Person类定义"/>
...
<CustomCodeBlock region="依赖属性应用场景扩展定义"/>

使用示例：

<CustomCodeBlock region="依赖属性应用场景示例"/>
