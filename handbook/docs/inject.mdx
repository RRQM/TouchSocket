...
`Inject()` 方法是 `TouchSocket` 框架提供的最小侵入式的方法，可以让任何 `ASP.NET Core` Web 后端项目瞬间支持所有 `TouchSocket` 框架特性。
...
`Inject()` 是在启动程序 `Program.cs` 中使用的，集成 `TouchSocket` 框架唯一方法，如：

```cs showLineNumbers  {6,18} title="TouchSocket.Web.Entry\Program.cs"
