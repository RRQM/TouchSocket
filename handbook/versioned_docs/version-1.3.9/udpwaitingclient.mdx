...
Udp的同步请求，有两种策略，一种则是在配置中设置**SetRemoteIPHost**，然后在**GetWaitingClient**时则不必再设置RemoteIPHost，这种情况一般适用于客户端。第二种则是不配置**SetRemoteIPHost**，然后在**GetWaitingClient**时设置RemoteIPHost，这种情况适用于服务器。
