...
### 37.3.3 安装 `TouchSocket.Tools.CommandLine` 包

为了方便管理工具包开发，`TouchSocket` 官方特意开发了 `TouchSocket.Tools.CommandLine` 包，帮助大家快速开发管理工具包。
...
- 不支持中文：[http://patorjk.com/software/taag/#p=display&f=Big&t=TouchSocket%20Tools](http://patorjk.com/software/taag/#p=display&f=Big&t=TouchSocket%20Tools)
...
using TouchSocket.Tools.CommandLine;
using System;
...
- `TouchSocket` 工具包提供了非常方便的 `Cli.Inject()` 方法，可以实现一次性完成所有初始化工作，只需要在 `Main` 方法调用即可
...
- `commandLineArgs`：编写测试命令，只需要写参数/操作符部分即可，如：`-v`，`-v -h --Name TouchSocket`
...
```bash showLineNumbers
dotnet hello-tools -n TouchSocket
```
...
<Project Sdk="Microsoft.NET.Sdk">
	...
	<PropertyGroup>
		...
		<PackageProjectUrl>https://TouchSocket.icu</PackageProjectUrl>
	</PropertyGroup>

	<ItemGroup>
		<PackageReference Include="TouchSocket.Tools.CommandLine" Version="2.6.0" />
	</ItemGroup>
...
为了简化包工具的开发，`TouchSocket.Tools.CommandLine` 的 `Cli` 静态类提供了很多方便的静态方法：
...
如果安装了 `Spectre.Console.Cli` 之后则无需安装 `TouchSocket.Tools.CommandLine` 拓展包了。
