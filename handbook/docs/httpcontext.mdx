...
`ASP.NET Core` 和 `TouchSocket` 提供了多种访问 `HttpContext` 的方式。
...
在 `TouchSocket` 框架中，默认已经注册了 `IHttpContextAccessor` 服务，所以我们可以通过构造函数注入该接口获取。
...
`TouchSocket` 框架基于 `HttpContext` 提供了一些常用的拓展方法。
...
**`Swagger` 默认不能记住授权信息，一旦刷新浏览器就自动清空**，所以 `TouchSocket` 提供了该拓展，即使刷新浏览器也能保持授权状态。
...
以下内容仅限 `TouchSocket 4.7.9 +` 版本使用。

:::

默认情况下，`ASP.NET Core` 不支持重复读取 `Body` 内容，`TouchSocket` 框架提供了拓展方法，需要按照以下步骤操作：
