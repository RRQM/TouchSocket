...
tags: [TouchSocket, furos, .net, .netcore, .net5, httpcontext]
---
...
在 `TouchSocket` 框架中，默认已经注册了 `IHttpContextAccessor` 服务，所以我们可以通过构造函数注入该对象获取。
...
`TouchSocket` 框架也提供了一些常用的 `HttpContext` 拓展方法
