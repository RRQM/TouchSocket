...
要回复`Websocket`消息，必须使用**HttpSessionClient**对象。

### 6.1 如何获取SessionClient？
...
```csharp showLineNumbers
HttpSessionClient[] SessionClients = service.GetClients();
...
```csharp showLineNumbers
SessionClient.WebSocket.SendAsync("Text");
```

### 6.3 发送二进制消息

```csharp showLineNumbers
SessionClient.WebSocket.SendAsync(new byte[10]);
```
...
SessionClient.WebSocket.SendAsync(frame);
```
