    ...
    public abstract class TcpDmtpSessionClient : TcpSessionClientBase, ITcpDmtpSessionClient
    {
        ...
        protected override Task OnTcpConnected(ConnectedEventArgs e)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    this.TryShutdown();
                    await base.CloseAsync("Handshak验证超时").ConfigureFalseAwait();
                }
            });
        ...
        {
            await this.m_privateConnecting(this, e).ConfigureFalseAwait();
            await base.OnTcpConnecting(e).ConfigureFalseAwait();
        }
